...
class BankImportService
{
    ...
    public function import(Workspace $workspace, int $userId, UploadedFile $file, ?array $selectedSignatures = null): BankStatementImport
    {
        ...
        try {
            ...
            $imported = 0; $duplicatesSkipped = 0; $expenseIndex = 0;
            $selectedLookup = is_array($selectedSignatures) ? array_fill_keys(array_values($selectedSignatures), true) : null;

            foreach ($transactions as $transaction) {
                if (($transaction['amount'] ?? 0) >= 0) continue;
                $signature = $this->buildExpenseSignature($transaction, $expenseIndex++);
                if ($selectedLookup !== null && ! isset($selectedLookup[$signature])) continue;
                $amount = abs((float) $transaction['amount']);
                $description = trim((string) $transaction['description']);
                if ($this->expenseAlreadyExists($workspace->id, $userId, $transaction['date'], $amount, $description)) { $duplicatesSkipped++; continue; }
    ...
    {
        if ($ext === 'ofx') return ['transactions' => $this->parseOfxTransactions($content), 'bank' => $this->detectBankFromOfx($content), 'source_file_type' => 'ofx'];
        $rows = $this->parseCsv($content); $bank = $this->detectBank($rows); $transactions = [];
        foreach ($rows as $i => $row) { if ($i === 0 && $this->looksLikeHeader($row)) continue; $parsed = $this->parseRow($row, $bank); if ($parsed) $transactions[] = $parsed; }
    ...
    {
        $content = mb_convert_encoding($content, 'UTF-8', 'UTF-8, ISO-8859-1, Windows-1252'); $transactions = [];
        ...
        foreach ($blocks[1] ?? [] as $block) {
            ...
            if ($description === '') $description = $this->extractOfxField($block, 'TRNTYPE') ?: 'Transacao OFX';
    ...
    {
        $value = trim($value);
        if (preg_match('/^(\d{8})/', $value, $m)) { try { $d = Carbon::createFromFormat('Ymd', $m[1]); if ($d && $d->year > 2000) return $d; } catch (\Throwable) {} }
    ...
    private function detectBankFromOfx(string $content): string
    {
        ...
        if (str_contains($lower, 'revolut')) return 'revolut';
        if (str_contains($lower, 'n26')) return 'n26';
        if (str_contains($lower, 'millennium') || str_contains($lower, 'bcp')) return 'millennium';
        if (str_contains($lower, 'caixa geral') || str_contains($lower, 'cgd')) return 'cgd';
        return 'ofx';
    }
    ...
    private function detectBank(array $rows): string
    {
        ...
        foreach (self::BANK_PATTERNS as $bank => $patterns) foreach ($patterns as $pattern) if (str_contains($header, $pattern)) return $bank;
        return 'generic';
    }
    ...
    private function parseRow(array $row, string $bank): ?array
    {
        ...
        return match ($bank) { 'revolut' => $this->parseRevolut($row), 'n26' => $this->parseN26($row), default => $this->parseGeneric($row) };
    }

    private function parseRevolut(array $row): ?array
    {
        $date = $this->parseDate($row[0] ?? $row[1] ?? ''); $amount = $this->parseAmount($row[2] ?? $row[3] ?? ''); $desc = $row[4] ?? $row[3] ?? $row[1] ?? 'Transação';
        return $date && $amount !== null ? ['date' => $date, 'amount' => $amount, 'description' => $desc] : null;
    }

    private function parseN26(array $row): ?array
    {
        $date = $this->parseDate($row[0] ?? ''); $amount = $this->parseAmount($row[1] ?? $row[2] ?? ''); $desc = $row[3] ?? $row[2] ?? 'Transação';
        return $date && $amount !== null ? ['date' => $date, 'amount' => $amount, 'description' => $desc] : null;
    }
    ...
    private function parseGeneric(array $row): ?array
    {
        ...
        foreach ($row as $cell) {
            if (! $date && $d = $this->parseDate($cell)) $date = $d;
            elseif ($amount === null && ($a = $this->parseAmount($cell)) !== null) $amount = $a;
            elseif (strlen($cell) > 3 && ! is_numeric(str_replace([',', '.', '-', ' '], '', $cell))) $desc = $cell;
        }
    ...
    {
        $value = trim($value);
        foreach (['Y-m-d', 'd/m/Y', 'd-m-Y', 'd.m.Y', 'Y/m/d'] as $format) { try { $d = Carbon::createFromFormat($format, $value); if ($d && $d->year > 2000) return $d; } catch (\Throwable) {} }
        return null;
    }
    ...
    private function parseAmount(string $value): ?float
    {
        ...
        $lastComma = strrpos($value, ','); $lastDot = strrpos($value, '.');
        if ($lastComma !== false && $lastDot !== false) {
            if ($lastComma > $lastDot) { $value = str_replace('.', '', $value); $value = str_replace(',', '.', $value); }
            ...
            if ($fractionLength !== 1 && $fractionLength !== 2) $value = str_replace('.', '', $value);
        }
    ...
    private function categorize(int $workspaceId, int $userId, string $description, array $categories): ?int
    {
        ...
        if ($customRuleCategoryId) return $customRuleCategoryId;
        ...
        foreach ($keywords as $catName => $words) foreach ($words as $word) if (str_contains($desc, $word)) foreach ($categories as $id => $name) if (str_contains(strtolower($name), $catName) || strtolower($name) === $catName) return $id;
        if ($this->hasAiKey()) return $this->categorizeWithAi($description, $categories);
        return null;
    }
    ...
    private function categorizeWithRules(int $workspaceId, int $userId, string $descriptionLower, array $validCategoryIds): ?int
    {
        ...
        foreach ($rules as $rule) {
            ...
            if ($keyword === '' || ! isset($validLookup[(int) $rule->category_id])) continue;
            if (str_contains($descriptionLower, $keyword)) return (int) $rule->category_id;
        }
        return null;
    }
    ...
    private function categorizeWithAi(string $description, array $categories): ?int
    {
        try {
            ...
            if (strcasecmp($name, 'UNKNOWN') === 0) return null;
            foreach ($categories as $id => $catName) if (strcasecmp($catName, $name) === 0) return $id;
        } catch (\Throwable) {
        }
        return null;
    }
