...
class BusinessSettlementService
{
    public function receiveInvoice(Invoice $invoice, float $amount, ?int $bankAccountId = null, ?string $reference = null, ?string $paidAt = null): PaymentAllocation
    {
        ...
        return DB::transaction(function () use ($invoice, $amount, $bankAccountId, $reference, $paidAt, $workspace) {
            ...
            $invoice->update([
                'amount_paid' => $newPaid,
                'status' => $newPaid >= $netDue ? 'paga' : 'pendente',
                'paid_at' => $newPaid >= $netDue ? now() : null,
            ]);
    ...
    {
        $workspace = app(BusinessAccessService::class)->assertWorkspace();
        if ((int) $expense->workspace_id !== (int) $workspace->id) {
            abort(404);
        }
        $this->assertManageFinancials($workspace);
        ...
        return DB::transaction(function () use ($expense, $amount, $bankAccountId, $reference, $paidAt, $workspace) {
            ...
            if ($amount <= 0 || $amount > $outstanding) {
                throw ValidationException::withMessages(['amount' => 'O pagamento excede o saldo da despesa.']);
            }
            ...
            $allocation = PaymentAllocation::create([
                ...
                'bank_account_id' => $bankAccountId,
                'amount' => $amount,
                'currency' => $expense->currency ?: $workspace->currency,
                'paid_at' => $paidAt ?: now()->toDateString(),
                'reference' => $reference,
            ]);
            ...
            $expense->update([
                'amount_paid' => $newPaid,
                'status' => $newPaid >= (float) $expense->amount ? 'paga' : $expense->status,
            ]);
    ...
    public function issueCreditNote(Invoice $invoice, float $amountExclVat, float $vatAmount, string $reason, string $number): CreditNote
    {
        ...
        return DB::transaction(function () use ($invoice, $amountExclVat, $vatAmount, $reason, $number, $workspace) {
            ...
            $available = max(0, (float) $invoice->total_amount - $alreadyCredited - (float) $invoice->amount_paid);

            if ($totalCredit <= 0 || $totalCredit > $available) {
                throw ValidationException::withMessages(['amount' => 'A nota de crédito excede o saldo ainda não liquidado da fatura.']);
            }
    ...
    {
        $workspace = app(BusinessAccessService::class)->assertWorkspace();
        if ((int) $transaction->workspace_id !== (int) $workspace->id) {
            abort(404);
        }
        $this->assertManageFinancials($workspace);

        if ($transaction->status === 'reconciled') {
            throw ValidationException::withMessages(['transactionId' => 'Este movimento bancário já está reconciliado.']);
        }

        if (! in_array($matchedType, ['payment_allocation', 'invoice', 'expense'], true)) {
            throw ValidationException::withMessages(['matched_type' => 'Tipo de reconciliação inválido.']);
        }

        $targetAmount = match ($matchedType) {
            'payment_allocation' => PaymentAllocation::where('workspace_id', $workspace->id)->whereKey($matchedId)->value('amount'),
            'invoice' => Invoice::where('workspace_id', $workspace->id)->whereKey($matchedId)->value('total_amount'),
            'expense' => Expense::where('workspace_id', $workspace->id)->whereKey($matchedId)->value('amount'),
        };
        ...
        if (abs(abs((float) $transaction->amount) - abs((float) $targetAmount)) > 0.01) {
            throw ValidationException::withMessages([
                'matchedId' => 'O valor do movimento bancário não corresponde ao valor do documento selecionado. Faça a correspondência com o pagamento correto.',
            ]);
        }
    ...
    private function assertInvoiceWorkspace(Invoice $invoice, int $workspaceId): void
    {
    ...
    private function assertManageFinancials($workspace): void
    {
