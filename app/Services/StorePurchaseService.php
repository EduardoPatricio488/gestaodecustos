...
class StorePurchaseService
{
    ...
    public function completePurchase(StoreProduct $product, float $amountPaid, string $paymentMethod = 'simulated', ?StoreCoupon $coupon = null, float $discount = 0, ?int $userId = null, ?string $stripeSessionId = null): StorePurchase
    {
        ...
        return DB::transaction(function () use ($product, $amountPaid, $paymentMethod, $coupon, $discount, $userId, $stripeSessionId) {
            ...
            if ($product->points_reward > 0) {
                ...
                if ($user) { DB::table('users')->where('id', $user->id)->increment('points', $product->points_reward); $user->addXp($product->points_reward * 10); }
            }
            ...
            if ($user = User::find($userId)) app(StoreEntitlementService::class)->clearCache($user);
    ...
    {
        $userId = $userId ?? Auth::id(); $user = User::find($userId);
        if (! $user?->current_workspace_id || $amount <= 0) return null;
        $workspaceId = $user->current_workspace_id;
        $category = Category::where('workspace_id', $workspaceId)->where(fn ($q) => $q->where('slug', 'educacao')->orWhere('name', 'Educação'))->first();
        if (! $category) $category = Category::create(['user_id' => $userId, 'workspace_id' => $workspaceId, 'name' => 'Educação', 'slug' => Category::uniqueSlugFor('Educação', $workspaceId), 'icon' => 'academic-cap', 'color' => '#06b6d4', 'is_fixed' => true]);
    ...
    public function logActivity(string $action, string $description, array $metadata = [], ?int $userId = null): void
    {
        ...
        $user = User::find($userId); if (! $user) return;
    ...
    public function completeStoreCheckout(StoreCheckoutSession $pending, string $stripeSessionId): void
    {
        $purchases = DB::transaction(function () use ($pending, $stripeSessionId) {
            ...
            if (! $locked || $locked->status === 'completed') return collect();
            ...
            foreach ($locked->items as $index => $item) {
                ...
                if ($locked->add_expense_to_education && $purchase->wasRecentlyCreated) $this->recordEducationExpense($product, (float) $item['amount_paid'], $locked->user_id);
            }
        ...
        if ($user) {
            try { Mail::to($user->email)->send(new StorePurchaseReceiptMail($user, $purchases, $stripeSessionId)); }
            catch (\Exception $e) { Log::error("Falha ao enviar recibo da loja para utilizador {$user->id}: ".$e->getMessage()); }
        }
    }
}
