...
class BusinessFinancialMetrics
{
    public function forMonth(Workspace $workspace, ?Carbon $month = null): array
    {
        ...
        $paidRevenue = (float) PaymentAllocation::where('workspace_id', $workspace->id)
            ->whereNotNull('invoice_id')
            ->whereBetween('paid_at', [$month->toDateString(), $end->toDateString()])
            ->get()
        ...
        $creditNotes = (float) $workspace->creditNotes()
            ->whereBetween('issued_at', [$month->toDateString(), $end->toDateString()])
            ->where('status', 'issued')
            ->sum('amount_excl_vat');
        ...
        $receivables = (float) (clone $invoices)
            ->get()
            ->sum(fn ($invoice) => (float) $invoice->outstanding_amount * (
                (float) $invoice->total_amount_converted / max((float) $invoice->total_amount, 0.01)
            ));
        ...
        $overdueReceivables = (float) (clone $invoices)
            ...
            ->whereNotIn('status', ['paga', 'cancelada', 'anulada'])
            ->get()
            ->sum(fn ($invoice) => (float) $invoice->outstanding_amount * (
                (float) $invoice->total_amount_converted / max((float) $invoice->total_amount, 0.01)
            ));

        $cash = (float) $workspace->bankAccounts()
            ->where('type', '!=', 'credito')
            ->get()
        ...
        if (! $workspace->bankAccounts()->exists()) {
            $cash = (float) ($workspace->initial_capital ?? 0)
                + (float) PaymentAllocation::where('workspace_id', $workspace->id)->whereNotNull('invoice_id')->sum('amount')
                - (float) PaymentAllocation::where('workspace_id', $workspace->id)->whereNotNull('expense_id')->sum('amount');
        }
