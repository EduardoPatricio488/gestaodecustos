...
class AiBrainService
{
    ...
    public function chat(User $user, string $input, ?AiConversation $conversation = null, array $pageContext = []): array
    {
        ...
        try {
            for ($round = 0; $round < 5; $round++) {
                $response = $this->provider($messages, $round === 0 ? $forcedWriteTool : null);
            ...
            $conversation->update([
                ...
                'title' => $conversation->title ?: Str::limit($cleanInput, 60),
            ]);
            ...
            $conversation->messages()->create([
                'user_id' => $user->id,
                'role' => 'assistant',
                'content' => 'Não consegui concluir este registo. Os teus dados não foram inventados nem alterados.',
                'is_error' => true,
                'latency_ms' => (int) round((microtime(true) - $startedAt) * 1000),
            ]);
            throw $e;
    ...
    {
        // Financial memory is never global. Only workspace-scoped memories can
        // participate in financial reasoning; global memories are deliberately
        // excluded to prevent personal/business leakage.
    ...
    private function provider(array $messages, ?string $forcedWriteTool = null): array
    {
...
- Se o utilizador pedir para criar, adicionar, registar ou inserir algo, USA A TOOL DE ESCRITA CORRESPONDENTE. Não respondas apenas com instruções sobre como o utilizador pode fazê-lo manualmente.
- As tools de escrita criam uma pré-visualização. Depois da confirmação explícita do utilizador, a ação é executada no backend e o registo passa a existir na aplicação.
...
12. Responde de forma curta por defeito.
