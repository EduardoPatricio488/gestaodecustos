...
class StoreHub extends Component
{
    ...
    public function startEdit(int $id): void
    {
        ...
        $this->featuresRaw = is_array($product->features) ? implode("\n", $product->features) : '';
    }
    ...
    public function saveProduct(): void
    {
        ...
        $baseSlug = $slug;
        $i = 2;
        while (StoreProduct::where('slug', $slug)->when($this->editingProductId, fn ($q) => $q->where('id', '!=', $this->editingProductId))->exists()) {
            $slug = $baseSlug.'-'.$i++;
        }
    ...
    public function render(StoreAdminService $admin, StoreTabsService $tabsService)
    {
        ...
        if ($this->productSearch !== '') {
            $productsQuery->where(fn ($q) => $q->where('title', 'like', '%'.$this->productSearch.'%')->orWhere('type', 'like', '%'.$this->productSearch.'%'));
        }
        ...
        if ($this->purchaseSearch !== '') {
            $purchasesQuery->where(fn ($q) => $q->whereHas('user', fn ($u) => $u->where('name', 'like', '%'.$this->purchaseSearch.'%')->orWhere('email', 'like', '%'.$this->purchaseSearch.'%'))->orWhereHas('product', fn ($p) => $p->where('title', 'like', '%'.$this->purchaseSearch.'%')));
        }
