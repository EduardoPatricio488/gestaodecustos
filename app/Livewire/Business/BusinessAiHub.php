...
class BusinessAiHub extends Component
{
    ...
    private function buildInsights($workspace, array $metrics, $projects = null, float $risk = 0, int $lowStock = 0): array
    {
        ...
        $insights = [];
        if ($metrics['payroll'] > 0 && $metrics['cash'] < $metrics['payroll']) {
            $insights[] = ['type' => 'danger', 'title' => 'Pressão de tesouraria', 'text' => 'O saldo disponível está abaixo do custo salarial mensal registado.'];
        }
        if ($risk > 45) {
            $insights[] = ['type' => 'warning', 'title' => 'Concentração de receita', 'text' => 'Um cliente representa mais de 45% da receita recebida registada.'];
        }
        $inefficient = $projects->filter(fn ($p) => $p['hourly_profit'] > 0 && $p['hourly_profit'] < $this->targetHourlyRate)->count();
        if ($inefficient > 0) {
            $insights[] = ['type' => 'warning', 'title' => 'Projetos abaixo do objetivo', 'text' => "$inefficient projetos estão abaixo de {$this->targetHourlyRate}€/hora de resultado registado."];
        }
        if ($lowStock > 0) {
            $insights[] = ['type' => 'info', 'title' => 'Stock baixo', 'text' => "$lowStock artigos estão abaixo do nível mínimo configurado."];
        }
