...
use App\Mail\EmployeeDataUpdatedMail;
use App\Mail\EmployeeDeletedMail;
...
class TeamHub extends Component
{
    ...
    public function sendInviteEmail($id)
    {
        ...
        // 2. Enviar o E-mail
        try {
            Mail::to($email)->send(new WorkspaceInviteMail(
                $workspace->name,
    ...
    public function activateBusinessPlan($id)
    {
        ...
        try {
            ...
            Log::error('Erro ao enviar email de upgrade: '.$e->getMessage());
        }
        ...
        if ($this->editingId && $emp->user_id) {
            ...
            if ($userAccount) {
                ...
                try {
                    ...
                    Log::error('Falha ao notificar colaborador: '.$e->getMessage());
                }
    ...
    public function activate($id)
    {
        ...
        if ($emp->user) {
            try {
                Mail::to($emp->user->email)->send(new EmployeeReactivatedMail($emp->name, Auth::user()->currentWorkspace->name));
                ...
                Log::error('Erro ao enviar email de reativação: '.$e->getMessage());
            }
        }
    ...
    public function suspend($id)
    {
        ...
        if ($emp->user) {
            Mail::to($emp->user->email)->send(new EmployeeSuspendedMail($emp->name, Auth::user()->currentWorkspace->name));
    ...
    public function terminate($id)
    {
        ...
        if ($emp->user) {
            Mail::to($emp->user->email)->send(new EmployeeTerminatedMail($emp->name, Auth::user()->currentWorkspace->name));
    ...
    public function deleteEmployee($id)
    {
        ...
        if ($email) {
            Mail::to($email)->send(new EmployeeDeletedMail($name));
        }
