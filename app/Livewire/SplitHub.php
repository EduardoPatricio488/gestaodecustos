...
class SplitHub extends Component
{
    ...
    {
        if ($this->splitType !== 'equal' || empty($this->selectedUsers)) return;
    ...
    public function togglePaid(int $participantId): void
    {
        ...
        $split = $p->split;
        if ($split->creator_user_id !== auth()->id() && $p->user_id !== auth()->id()) return;
