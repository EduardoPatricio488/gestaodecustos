...
public function copyProfileLink(string $username)
{
    ...
    {
        $challenge = CommunityChallenge::where('is_active', true)->findOrFail($challengeId);

        CommunityChallengeParticipant::firstOrCreate(
            ['community_challenge_id' => $challenge->id, 'user_id' => auth()->id()],
            ['progress_pct' => 0, 'joined_at' => now()]
        );

        $this->dispatch('toast', text: 'Entraste no desafio "'.$challenge->title.'"!');
    }

    public function shareGoalProgress(int $goalId): void
    {
        ...
        SocialPost::create([
            'user_id' => auth()->id(),
            'workspace_id' => auth()->user()->current_workspace_id,
            'content' => "🎯 Meta: {$goal->name} — {$pct}% concluído! #metas #finanças",
            'visibility' => 'public',
        ]);
    ...
    public function render()
    {
        ...
        $myGoals = Goal::where('workspace_id', auth()->user()->current_workspace_id)
            ->where('user_id', auth()->id())
            ->whereColumn('current_amount', '<', 'target_amount')
            ->limit(5)
            ->get();

        return view('livewire.social.social-hub', compact('activeChallenges', 'myGoals'));
    }
}
