...
trait InteractsWithStore
{
    ...
    public function validateStoreCart(): bool
    {
        ...
        foreach ($cart->items() as $item) {
            if (! $this->canPurchase($item['product']->id, false)) {
                return false;
            }
        }
    ...
    private function canPurchase(int $productId, bool $notify = true): bool
    {
        ...
        if (! $user) {
            $message = 'Inicia sessão para comprar produtos.';
        } elseif ($product->type === 'plan') {
            $message = 'Os planos são geridos na área de subscrição.';
        } elseif (app(StoreCartService::class)->isOwned($productId)) {
            $message = 'Já tens este produto no teu inventário.';
        } elseif ($product->requires_business_plan && ! $user->isBusinessPlan()) {
            $message = 'Este produto requer o plano Business.';
        } elseif (($product->audience ?? 'both') === 'business' && ! $user->isBusinessPlan()) {
            $message = 'Este produto está disponível apenas para negócios.';
        }
