...
class InvestmentsHub extends Component
{
    ...
    private function analyzeWithOpenRouter(string $company, array $marketData): array
    {
        ...
        if (blank($apiKey)) return ['analysis_message' => 'A análise IA não está configurada neste ambiente.'];
        if (blank($marketData['price'] ?? null)) return ['analysis_message' => 'Não existem dados de mercado verificados suficientes para analisar esta empresa. A IA não irá inventar fundamentais, rácios ou uma recomendação.'];
        ...
        if (! is_array($json) || $json === []) return ['analysis_message' => 'A IA devolveu uma resposta sem dados estruturados para esta empresa.'];
        return $json;
    }
    ...
    private function analyzeWithGemini(string $company, array $marketData): array
    {
        ...
        if (blank($apiKey)) return ['analysis_message' => 'A análise Gemini não está configurada neste ambiente.'];
        if (blank($marketData['price'] ?? null)) return ['analysis_message' => 'Não existem dados de mercado verificados suficientes para analisar esta empresa.'];
        ...
        if (! $response->successful()) return ['analysis_message' => 'Não foi possível obter a análise Gemini neste momento.'];
    ...
    private function fetchMarketDataYahoo15(string $symbol): array
    {
        try {
            ...
            if (filled($key)) {
                ...
                if (is_array($body) && ! empty($body)) return $this->normalizeMarketData($body, $symbol);
            }
    ...
    private function fetchMarketDataFromYahooChart(string $symbol): array
    {
        try {
            ...
            if (! $response->successful()) return $this->fetchMarketDataFromAlphaVantage($symbol);
    ...
    private function fetchMarketDataFromAlphaVantage(string $symbol): array
    {
        ...
        if (blank($key)) return $this->fetchMarketDataFromStooq($symbol);
        try {
            ...
            if (! $response->successful() || blank($price)) return $this->fetchMarketDataFromStooq($symbol);
            return [
    ...
    private function fetchMarketDataFromStooq(string $symbol): array
    {
        ...
        if (! $response->successful()) return ['error' => 'Fonte de cotações indisponível.'];
        $rows = collect(preg_split('/\r\n|\r|\n/', trim($response->body())))->filter(fn ($line) => $line !== '' && ! str_starts_with($line, 'No data'))->values();
        if ($rows->count() < 2) return ['error' => 'Fonte de cotações indisponível.'];
    ...
    public function switchTab($tab): void { $this->tab = $tab; }
    public function setType(string $newType): void
    {
        ...
        if ($newType === 'Divida') {
            $this->average_price = 1.00; $this->fees = 0; $this->issuer = 'IGCP / Estado Português'; $this->broker = 'AforroNet'; $this->product_type = 'CA';
        }
    }
    public function toggleNetValues(): void { $this->showNetValues = ! $this->showNetValues; }
    public function setFilter(string $type): void { $this->filterType = $type; }
    public function updatedTotalAmount(): void { if (is_numeric($this->total_amount) && is_numeric($this->average_price) && $this->average_price > 0) $this->quantity = round((max(0, (float) $this->total_amount - (float) ($this->fees ?? 0))) / (float) $this->average_price, 4); }
    public function updatedQuantity(): void { if (is_numeric($this->quantity) && is_numeric($this->average_price)) $this->total_amount = round((float) $this->quantity * (float) $this->average_price + (float) ($this->fees ?? 0), 2); }
    public function updatedAveragePrice(): void { if ($this->total_amount > 0) $this->updatedTotalAmount(); elseif ($this->quantity > 0) $this->updatedQuantity(); }
    public function updatedFees(): void { if (is_numeric($this->quantity) && is_numeric($this->average_price)) $this->updatedQuantity(); }
    ...
    private function fetchCryptoPrices(array $symbols): array
    {
        ...
        return Cache::remember('coingecko_prices_'.md5($ids), self::CACHE_PRICES_TTL, function () use ($ids, $idMap) {
            try {
                ...
                if (! $res->ok()) return [];
                $prices = []; $reverseMap = array_flip($idMap);
                foreach ($res->json() as $id => $data) $prices[strtoupper($reverseMap[$id] ?? $id)] = $data['eur'] ?? null;
                return $prices;
    ...
    private function fetchFromYahoo(string $symbol): ?float
    {
        try {
            ...
            if ($currency === 'USD') { $rate = $this->getEurUsdRate(); if ($rate === null) return null; $price *= $rate; }
            elseif ($currency === 'GBP') { $rate = $this->getEurGbpRate(); if ($rate === null) return null; $price *= $rate; }
            elseif ($currency === 'GBP' || $currency === 'GBp') return null;
            return (float) $price;
    ...
    public function refreshPrices(): void
    {
        ...
        try {
            ...
            $debtAssets = $assets->where('type', 'Divida'); $updated = 0;
            foreach ($debtAssets as $debtAsset) {
                if (! $debtAsset->operation_date || ! $debtAsset->interest_rate) continue;
                $debtAsset->update(['current_price' => DebtInstrumentCalculator::process($debtAsset)]); $updated++;
            }
            ...
            $stockAssets = $assets->whereIn('type', ['Acao', 'ETF', 'Fundo']); $stockPrices = [];
            foreach ($stockAssets->pluck('symbol')->unique() as $sym) { Cache::forget('stock_price_'.strtoupper($sym)); $price = $this->fetchStockPrice($sym); if ($price !== null) $stockPrices[strtoupper($sym)] = $price; }
            $ids = collect($cryptoSymbols)->map(fn ($s) => $this->coinGeckoId($s))->implode(','); if ($ids) Cache::forget('coingecko_prices_'.md5($ids));
            foreach ($assets->whereIn('type', ['Acao', 'ETF', 'Fundo', 'Cripto']) as $asset) { $ticker = strtoupper($asset->symbol); if (isset($cryptoPrices[$ticker]) && $cryptoPrices[$ticker] > 0) { $asset->update(['current_price' => $cryptoPrices[$ticker]]); $updated++; } elseif (isset($stockPrices[$ticker]) && $stockPrices[$ticker] > 0) { $asset->update(['current_price' => $stockPrices[$ticker]]); $updated++; } }
            $this->lastUpdated = now()->format('H:i'); $this->dispatch('toast', text: "{$updated} ativos sincronizados com o mercado.");
        } catch (\Throwable $e) { $this->dispatch('toast', text: 'Erro ao sincronizar preços: '.$e->getMessage(), variant: 'error'); }
        $this->isRefreshing = false;
    }

    private function coinGeckoId(string $symbol): string { $map = ['BTC' => 'bitcoin', 'ETH' => 'ethereum', 'SOL' => 'solana', 'ADA' => 'cardano', 'DOT' => 'polkadot', 'MATIC' => 'matic-network']; return $map[strtoupper($symbol)] ?? strtolower($symbol); }
    private function coinGeckoSymbolFromId(string $id): string { $map = ['bitcoin' => 'BTC', 'ethereum' => 'ETH', 'solana' => 'SOL', 'cardano' => 'ADA', 'matic-network' => 'MATIC']; return $map[strtolower($id)] ?? strtoupper($id); }
    ...
    public function createAsset(): void
    {
        ...
        $this->type = 'Acao'; $this->fees = 0; $this->operation_date = now()->toDateString(); $this->dispatch('modal-show-add-investment');
    }

    public function editAsset(int $id): void
    {
        $this->editingId = $id; $asset = Investment::where('workspace_id', Auth::user()->current_workspace_id)->findOrFail($id);
        foreach (['symbol','name','isin','type','exchange','network','provider','broker','quantity','average_price','fees','interest_rate','loyalty_bonus','issuer','series'] as $field) $this->{$field} = $asset->{$field};
        $this->operation_date = $asset->operation_date?->toDateString(); $this->total_amount = round((float) $asset->quantity * (float) $asset->average_price + (float) ($asset->fees ?? 0), 2); $this->capitalization_date = $asset->capitalization_date?->toDateString(); $this->product_type = $asset->product_type ?? 'CA'; $this->dispatch('modal-show-add-investment');
    }
    ...
    public function save(): void
    {
        ...
        if ($this->editingId) { Investment::where('workspace_id', Auth::user()->current_workspace_id)->findOrFail($this->editingId)->update($data); $msg = 'Ativo atualizado no cofre!'; }
        else { $data['current_price'] = (float) $this->average_price; Investment::create($data); $user = Auth::user(); $user->awardXp(25, 'investimento registado'); $msg = 'Novo capital registado com sucesso! '.$user->xpToastText(25, 'investimento registado'); }
        $this->reset(['symbol', 'name', 'isin', 'quantity', 'average_price', 'fees', 'total_amount', 'exchange', 'network', 'provider', 'broker', 'operation_date', 'editingId', 'interest_rate', 'loyalty_bonus', 'capitalization_date', 'issuer', 'series', 'product_type']); $this->dispatch('modal-close-add-investment'); $this->dispatch('toast', text: $msg);
    }

    public function deleteAsset(int $id): void { Investment::where('user_id', Auth::id())->where('workspace_id', Auth::user()->current_workspace_id)->findOrFail($id)->delete(); $this->dispatch('toast', text: 'Ativo removido do portefólio.'); }

    public function render()
    {
        $query = Investment::where('workspace_id', Auth::user()->current_workspace_id);
        if ($this->search) { $term = '%'.trim($this->search).'%'; $query->where(fn ($q) => $q->where('symbol', 'like', $term)->orWhere('name', 'like', $term)); }
        if ($this->filterType !== 'Todos') $query->where('type', $this->filterType);
        $myAssets = $query->latest()->get()->map(function ($asset) { $cost = (float) $asset->quantity * (float) $asset->average_price + (float) ($asset->fees ?? 0); $currentValue = (float) $asset->quantity * ($asset->current_price ?: (float) $asset->average_price); $asset->cost = $cost; $asset->current_value = $currentValue; $asset->pnl = $currentValue - $cost; $asset->pnl_percent = $cost > 0 ? ($asset->pnl / $cost) * 100 : 0; return $asset; });
        $totalInvested = $myAssets->sum('cost'); $currentPortfolioValue = $myAssets->sum('current_value'); $totalEstimatedTax = $myAssets->sum(fn ($asset) => ($asset->type !== 'Divida' && $asset->pnl > 0) ? $asset->pnl * 0.28 : 0); $totalProfit = $currentPortfolioValue - $totalInvested; $totalPnlPct = $totalInvested > 0 ? ($totalProfit / $totalInvested) * 100 : 0; $displayValue = $this->showNetValues ? ($currentPortfolioValue - $totalEstimatedTax) : $currentPortfolioValue; $displayProfit = $this->showNetValues ? ($totalProfit - $totalEstimatedTax) : $totalProfit;
    ...
    private function buildMarketTicker(): array
    {
        ...
        $spy = Cache::remember('ticker_spy', self::CACHE_TICKER_TTL, function () {
            try {
                $res = Http::timeout(6)->withHeaders(['User-Agent' => 'Mozilla/5.0'])->get('https://query1.finance.yahoo.com/v8/finance/chart/SPY', ['interval' => '1d', 'range' => '1d']); $meta = $res->json()['chart']['result'][0]['meta'] ?? []; $price = $meta['regularMarketPrice'] ?? null; $prev = $meta['chartPreviousClose'] ?? null; if (! $price) return null; $eurRate = $this->getEurUsdRate(); if ($eurRate === null) return null; $priceEur = $price * $eurRate; $prevEur = $prev ? $prev * $eurRate : null; $change = $prevEur ? (($priceEur - $prevEur) / $prevEur) * 100 : 0; return ['price' => number_format($priceEur, 2, '.', ' '), 'change' => ($change >= 0 ? '+' : '').number_format($change, 2).'%'];
            } catch (\Throwable) { return null; }
        });
        ...
        $xtb = Cache::remember('ticker_xtb', self::CACHE_TICKER_TTL, function () {
            try {
                $res = Http::timeout(6)->withHeaders(['User-Agent' => 'Mozilla/5.0'])->get('https://query1.finance.yahoo.com/v8/finance/chart/XTB.WA', ['interval' => '1d', 'range' => '1d']); $meta = $res->json()['chart']['result'][0]['meta'] ?? []; $price = $meta['regularMarketPrice'] ?? null; $prev = $meta['chartPreviousClose'] ?? null; if (! $price) return null;
                $rate = Cache::remember('fx_eur_pln', 3600, function () { try { $res = Http::timeout(5)->get('https://api.frankfurter.app/latest', ['from' => 'PLN', 'to' => 'EUR']); $value = $res->json()['rates']['EUR'] ?? null; return is_numeric($value) ? (float) $value : null; } catch (\Throwable) { return null; } });
                if ($rate === null) return null; $priceEur = $price * $rate; $prevEur = $prev ? $prev * $rate : null; $change = $prevEur ? (($priceEur - $prevEur) / $prevEur) * 100 : 0; return ['price' => number_format($priceEur, 2, '.', ' '), 'change' => ($change >= 0 ? '+' : '').number_format($change, 2).'%'];
            } catch (\Throwable) { return null; }
        });
        if ($xtb) $ticker['XTB'] = $xtb;

        $crypto = Cache::remember('ticker_crypto_top', self::CACHE_TICKER_TTL, function () {
            try {
                $res = Http::timeout(8)->get('https://api.coingecko.com/api/v3/simple/price', ['ids' => 'bitcoin,ethereum,solana', 'vs_currencies' => 'eur', 'include_24hr_change' => 'true']); $data = $res->json();
