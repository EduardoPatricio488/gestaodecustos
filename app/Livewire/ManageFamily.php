...
class ManageFamily extends Component
{
    ...
    public function updatePrivileges()
    {
        ...
        FamilyBudgetPermission::where('workspace_id', $workspaceId)
            ->where('user_id', $this->permUserId)
            ->whereNotNull('category_id')
            ->delete();

        foreach ($this->selectedCategories as $catId) {
            FamilyBudgetPermission::create([
                'workspace_id' => $workspaceId,
                'user_id' => $this->permUserId,
                'category_id' => $catId,
            ]);
    ...
    public function updatedPermUserId($id)
    {
        ...
        $this->selectedCategories = FamilyBudgetPermission::where('user_id', $id)
        ...
        $perm = FamilyBudgetPermission::where('user_id', $id)
            ->where('workspace_id', $workspaceId)
    ...
    {
        if (! $this->iAmAdmin || $userId === auth()->id()) {
            return;
        }
