...
class StravaWidget extends Component
{
    ...
    public function render()
    {
        ...
        if ($device) {
            try {
                $athlete = Cache::remember("strava:athlete:{$device->id}", 300, fn () => $strava->athlete($device)
                );

                if (! empty($athlete['id'])) {
                    $stats = Cache::remember("strava:stats:{$device->id}", 300, fn () => $strava->athleteStats($device, $athlete['id'])
                    );
                }

                $activities = Cache::remember("strava:activities:{$device->id}", 120, fn () => $strava->activities($device, 15)
                );
            } catch (\Throwable $e) {
