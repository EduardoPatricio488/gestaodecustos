...
trait BelongsToWorkspace
{
    ...
    protected static function assertMutationAllowed($model, string $operation): void
    {
        ...
        if ($role === 'viewer') throw new AuthorizationException('Este perfil tem acesso apenas de leitura.');

        $modelClass = class_basename($model);
        $isOwnExpense = $modelClass === 'Expense' && (int) ($model->user_id ?? 0) === (int) $user->id;
        if ($modelClass === 'Expense' && $role === 'employee' && $isOwnExpense && $operation !== 'delete') return;

        if ($modelClass === 'Absence') {
            $employee = Employee::withoutGlobalScopes()->find($model->employee_id);
            if ($employee && (int) $employee->workspace_id === (int) $workspace->id && (int) $employee->user_id === (int) $user->id && $operation !== 'delete') return;
