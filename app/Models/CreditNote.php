...
class CreditNote extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (CreditNote $note): void {
            ...
            if ($note->amount_excl_vat <= 0 || $note->vat_amount < 0) {
                throw new \DomainException('Valores da nota de crédito inválidos.');
            }
            $invoiceWorkspace = Invoice::withoutGlobalScopes()->whereKey($note->invoice_id)->value('workspace_id');
            if ((int) $invoiceWorkspace !== (int) $note->workspace_id) {
                throw new \DomainException('A fatura não pertence à empresa.');
            }
        });
    }
