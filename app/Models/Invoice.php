...
class Invoice extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (Invoice $invoice): void {
            if (! $invoice->workspace_id) return;
            ...
            if ($base <= 0 || $vat < 0 || (float) ($invoice->amount_paid ?? 0) < 0 || (float) ($invoice->amount_credited ?? 0) < 0) throw new DomainException('Os valores da fatura são inválidos.');
            if ((float) $invoice->amount_paid + (float) $invoice->amount_credited > $total + 0.01) throw new DomainException('Pagamentos e notas de crédito excedem o total da fatura.');
            ...
            if ($invoice->client_id) {
                ...
                if ((int) $clientWorkspaceId !== (int) $invoice->workspace_id) throw new DomainException('O cliente selecionado não pertence à empresa.');
            }
            if ($invoice->cost_center_id && (int) CostCenter::withoutGlobalScopes()->whereKey($invoice->cost_center_id)->value('workspace_id') !== (int) $invoice->workspace_id) throw new DomainException('O centro de custo não pertence à empresa.');
            if ($invoice->isDirty('status') && $invoice->status === 'paga' && ! $invoice->paid_at) $invoice->paid_at = now();
            if ($invoice->isDirty('status') && $invoice->status !== 'paga' && (float) $invoice->amount_paid < $total - (float) $invoice->amount_credited) $invoice->paid_at = null;
            $invoice->forceFill([
    ...
    public function getOutstandingAmountAttribute(): float { return max(0, round((float) $this->total_amount - (float) $this->amount_paid - (float) $this->amount_credited, 2)); }
}
