...
class Expense extends Model
{
    ...
    protected static function booted(): void
    {
        static::saving(function (Expense $expense): void {
            ...
            if ($amount <= 0 || $vat < 0 || $vat > $amount || $paid < 0 || $paid > $amount + 0.01) throw new DomainException('Os valores da despesa são inválidos.');
            if ($expense->category_id && (int) Category::withoutGlobalScopes()->whereKey($expense->category_id)->value('workspace_id') !== (int) $expense->workspace_id) throw new DomainException('A categoria selecionada não pertence à empresa.');
            if ($expense->supplier_id && (int) Supplier::withoutGlobalScopes()->whereKey($expense->supplier_id)->value('workspace_id') !== (int) $expense->workspace_id) throw new DomainException('O fornecedor selecionado não pertence à empresa.');
            if ($expense->cost_center_id && (int) CostCenter::withoutGlobalScopes()->whereKey($expense->cost_center_id)->value('workspace_id') !== (int) $expense->workspace_id) throw new DomainException('O centro de custo não pertence à empresa.');
