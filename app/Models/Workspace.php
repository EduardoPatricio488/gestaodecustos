...
class Workspace extends Model
{
    ...
    public function getTypeText(): string
    {
        return match ($this->type) {
            'personal' => 'Conta Individual','couple' => 'Conta Partilhada (Casal)','family' => 'Conta Familiar','business','company' => 'Gestão Empresarial',default => 'Outro'
        };
    }
    ...
    public function getBurnRate(): float
    {
        ...
        return round($total / max(1, $months) + (float) $this->employees()->where('active', true)->where('suspended', false)->whereNull('terminated_at')->sum('salary'), 2);
    }
...
return round((float) ($this->initial_capital ?? 0) + (float) $this->paymentAllocations()->whereNotNull('invoice_id')->sum('amount') - (float) $this->paymentAllocations()->whereNotNull('expense_id')->sum('amount'), 2);
    ...
    {
        $burn = $this->getBurnRate();
        $cash = $this->getLiquidezAtual();
        if ($burn <= 0) {
            return '∞';
        } if ($cash <= 0) {
            return '0 meses';
...
return number_format($cash / $burn, 1).' meses';
    ...
    {
        return app(FinanceScoreService::class)->calculate($this)['score'];
    }
}
