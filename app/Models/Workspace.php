...
{
    protected $fillable = ['name','type','owner_id','invite_code','legal_name','logo_path','tax_number','industry','currency','initial_capital','country_code','vat_rate','vat_regime','audit_token','audit_access_code','audit_token_expires_at','audit_token_revoked_at','audit_token_purpose','recruitment_extra_info','business_email','plan','plan_expires_at','address','recruitment_active','recruitment_description','recruitment_announcement','recruitment_vacancies','fiscal_year_start'];
    ...
    public function generateInviteCode() { if (! $this->invite_code) { $prefix=strtoupper(substr(preg_replace('/[^A-Za-z0-9]/','',$this->name),0,3)); $this->invite_code=$prefix.'-'.strtoupper(bin2hex(random_bytes(3))); $this->save(); } return $this->invite_code; }
    public function getLogoUrlAttribute(): string { if (! $this->logo_path) return 'https://ui-avatars.com/api/?name='.urlencode($this->name).'&color=10b981&background=ecfdf5&bold=true'; return Storage::url(preg_replace('#^/?storage/#','',$this->logo_path)); }
    ...
    public function bankTransactions(): HasMany { return $this->hasMany(BankTransaction::class); }
    ...
    public function getBurnRate(): float { $query=$this->expenses()->where('is_company',true)->where('spent_at','>=',now()->subMonths(3)); $total=(float)$query->sum('amount'); $months=$query->pluck('spent_at')->map(fn($date)=>Carbon::parse($date)->format('Y-m'))->unique()->count(); return round($total/max(1,$months)+(float)$this->employees()->where('active',true)->where('suspended',false)->whereNull('terminated_at')->sum('salary'),2); }
    public function getLiquidezAtual(): float { if($this->bankAccounts()->exists()) return round((float)$this->bankAccounts()->where('type','!=','credito')->get()->sum(fn($account)=>(float)$account->current_balance),2); return round((float)($this->initial_capital??0)+(float)$this->paymentAllocations()->whereNotNull('invoice_id')->sum('amount')-(float)$this->paymentAllocations()->whereNotNull('expense_id')->sum('amount'),2); }
