<div class="max-w-7xl mx-auto space-y-10 pb-32 px-4" x-data="{ modal: @entangle('openModal') }">
    ...
    {{-- ── 1. DASHBOARD DE ESTATÍSTICAS (KPIs) ── --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-6">
        @foreach ([
            ...
            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-{{ $color }}-100 flex flex-col justify-between relative overflow-hidden group">
                <div class="absolute -right-4 -bottom-4 opacity-5 group-hover:scale-110 transition-transform">
                    <flux:icon :name="$icon" class="size-24 text-{{ $color }}-600" />
                </div>
                <p class="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">{{ $label }}</p>
                <h3 class="text-4xl font-black mt-2 text-{{ $color }}-600 tracking-tighter">{{ $value }}</h3>
            </div>
        @endforeach
    </div>
   ...
   {{-- ── 2. CABEÇALHO DE CONTROLO (TOTALMENTE RESPONSIVO) ── --}}
    ...
    {{-- ── 4. MODAL ALPINE (RESPONSIVO) ── --}}
<div x-show="modal" x-cloak
     class="fixed inset-0 z-[300] flex items-center justify-center p-3 sm:p-6 bg-zinc-950/60 backdrop-blur-xl">
    ...
    <div @click.away="modal = false"
         ...
         class="relative w-full max-w-2xl bg-white rounded-[3.5rem] shadow-2xl overflow-hidden border border-emerald-500/20
                max-h-[90vh] flex flex-col transform-gpu">

        {{-- HEADER --}}
        <div class="shrink-0 px-8 py-6 border-b border-zinc-100 flex justify-between items-center bg-zinc-50/50">
            <div>
                <h2 class="text-2xl font-black text-zinc-900 uppercase italic leading-none tracking-tighter">
                    Configurar Lembrete
                </h2>
                <p class="text-[9px] text-emerald-600 font-bold uppercase mt-2 tracking-widest">
                    Agendamento Inteligente Finance Connect
                </p>
            </div>

            <button @click="modal = false"
                    class="p-3 bg-white rounded-2xl shadow-sm text-zinc-400 hover:text-zinc-900 transition-all">
        ...
        <form wire:submit.prevent="saveReminder" class="flex flex-col min-h-0 flex-1">
            ...
            <div class="min-h-0 flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-6 space-y-5">
                ...
                {{-- Título --}}
                <div class="relative">
                    <label class="absolute left-4 -top-2.5 px-2 bg-white text-[10px] font-bold uppercase tracking-widest text-emerald-600 z-10">
                        Título do Evento
                    </label>
                    <input type="text" wire:model="title"
                           placeholder="Ex: Liquidação de Dividendos"
                           class="w-full bg-zinc-50 border-2 border-zinc-200 focus:border-emerald-500 rounded-2xl py-4 px-5 text-sm font-bold text-zinc-900 outline-none transition-all">
                    @error('title')
                ...
                {{-- Agendamento --}}
                <div class="relative">
                    <label class="absolute left-4 -top-2.5 px-2 bg-white text-[10px] font-bold uppercase tracking-widest text-zinc-400 z-10">
                        Data e Hora
                    </label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="relative">
                        ...
                        <select wire:model="priority"
                                class="w-full bg-zinc-50 border-2 border-zinc-200 focus:border-emerald-500 rounded-2xl py-4 px-5 text-sm font-bold text-zinc-900 outline-none transition-all appearance-none">
                            <option value="low">🟢 Baixa</option>
                            <option value="medium">🟡 Média</option>
                            <option value="high">🔴 Alta</option>
                        </select>
                ...
                {{-- Categoria --}}
                <div class="space-y-3">
                    <p class="text-[10px] font-bold uppercase tracking-widest text-zinc-400 px-1">
                        Categoria
                    </p>
                        ...
                        @foreach(['finance' => 'Finanças', 'personal' => 'Pessoal', 'work' => 'Trabalho', 'health' => 'Saúde'] as $val => $l)
                            <button type="button"
                                    ...
                                    {{ $category === $val
                                        ...
                                        : 'bg-zinc-50 border-zinc-200 text-zinc-500 hover:border-emerald-500' }}">
                                {{ $l }}
                            </button>
                        @endforeach
                    </div>
                ...
                <div class="relative">
                    ...
                    <textarea wire:model="notes" rows="3"
                              ...
                              class="w-full bg-zinc-50 border-2 border-zinc-200 focus:border-emerald-500 rounded-2xl py-4 px-5 text-sm font-medium text-zinc-700 resize-none outline-none transition-all placeholder:text-zinc-400"></textarea>
                </div>

            </div>

            {{-- FOOTER --}}
            <div class="shrink-0 px-8 py-5 flex flex-col sm:flex-row gap-3 border-t border-zinc-100 bg-white">
                <button type="button"
                        @click="modal = false"
                        class="w-full h-14 rounded-2xl text-zinc-500 hover:text-zinc-800
                               hover:bg-zinc-100 font-bold uppercase text-xs tracking-widest
                ...
                <button type="submit"
                        ...
                        class="w-full h-14 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white
                               font-black uppercase text-xs tracking-widest shadow-xl shadow-emerald-500/30
