<div class="space-y-10 pb-24"
    ...
    x-init="initChart({{ json_encode($results['chartLabels']) }}, {{ json_encode($results['chartNominal']) }}, {{ json_encode($results['chartReal']) }})"
    x-on:livewire:navigated.window="initChart({{ json_encode($results['chartLabels']) }}, {{ json_encode($results['chartNominal']) }}, {{ json_encode($results['chartReal']) }})"
>
        ...
        <div class="flex items-center gap-6">
            <div class="relative">
                ...
                <div class="relative p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] shadow-2xl">
                    <flux:icon name="clock" class="w-10 h-10 text-indigo-600" />
                </div>
            </div>
            <div>
                <div class="flex items-center gap-3 flex-wrap">
                    <h1 class="text-2xl sm:text-3xl md:text-4xl font-black dark:text-white uppercase tracking-tighter italic leading-none">Simulador de Reforma</h1>
                    <flux:badge variant="neutral" class="bg-zinc-100 dark:bg-zinc-800 text-[9px] font-black uppercase tracking-widest border-none px-3 py-1">Planeamento Futuro</flux:badge>
                </div>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
        ...
        <div class="xl:col-span-8 space-y-6">
            ...
            {{-- STATUS --}}
            <div class="relative overflow-hidden p-8 rounded-[2.5rem] shadow-2xl border {{ $results['onTrack'] ? 'bg-emerald-600 border-emerald-700' : 'bg-zinc-950 border-zinc-800' }}">
                <div class="absolute -right-10 -top-10 size-40 {{ $results['onTrack'] ? 'bg-white/10' : 'bg-indigo-500/10' }} blur-3xl rounded-full"></div>
                <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.3em] {{ $results['onTrack'] ? 'text-emerald-100' : 'text-indigo-400' }} mb-2">
                            {{ $results['onTrack'] ? '✅ No Caminho Certo' : '⚠️ Atenção: Défice Previsto' }}
                        </p>
                        <h2 class="text-6xl font-black text-white tracking-tighter italic leading-none">
                            {{ number_format($results['nominalCapital'], 0, ',', '.') }}<small class="text-2xl ml-1">€</small>
                        </h2>
                        <p class="text-[11px] font-bold text-white/60 mt-2 uppercase tracking-widest">
                            Capital acumulado aos {{ $retirementAge }} anos · {{ $results['years'] }} anos de poupança
                        </p>
                    </div>
                    <div class="text-right shrink-0">
                        <p class="text-[9px] font-black uppercase tracking-widest {{ $results['onTrack'] ? 'text-emerald-100' : 'text-zinc-500' }} mb-1">Rendimento Mensal Est.</p>
                        <p class="text-3xl font-black text-white italic">{{ number_format($results['monthlyIncome'], 0, ',', '.') }}€</p>
                        <p class="text-[9px] {{ $results['onTrack'] ? 'text-emerald-200' : 'text-zinc-500' }} font-bold mt-1">
                            Objetivo: {{ number_format($targetMonthlyIncome, 0, ',', '.') }}€/mês
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <p class="text-2xl font-black text-emerald-500 tracking-tighter italic">{{ number_format($results['realCapital'], 0, ',', '.') }}</p>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <p class="text-2xl font-black text-indigo-500 tracking-tighter italic">{{ number_format($results['realMonthlyIncome'], 0, ',', '.') }}€</p>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <p class="text-2xl font-black dark:text-white tracking-tighter italic">{{ number_format($results['totalContributed'], 0, ',', '.') }}€</p>
                ...
                <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 rounded-[2rem] shadow-sm">
                    ...
                    <p class="text-2xl font-black text-amber-500 tracking-tighter italic">{{ number_format(max(0, $results['totalGrowth']), 0, ',', '.') }}€</p>
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 shadow-sm"
                ...
                x-on:livewire:update.window="
                    $nextTick(() => initChart(
                        {{ json_encode($results['chartLabels']) }},
                        {{ json_encode($results['chartNominal']) }},
                        {{ json_encode($results['chartReal']) }}
                    ))
                ">
                ...
                <div class="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-[2rem] p-6 flex gap-5 items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-amber-700 dark:text-amber-300 mt-1">
                            Para receber <strong>{{ number_format($targetMonthlyIncome, 0, ',', '.') }}€/mês</strong> na reforma, precisas de poupar
                            <strong>{{ number_format($results['requiredMonthly'], 0, ',', '.') }}€/mês</strong> em vez de
                            {{ number_format($monthlyContribution, 0, ',', '.') }}€.
                            Défice atual: <strong class="text-red-600">{{ number_format(abs($results['gap']), 0, ',', '.') }}€/mês</strong>.
                        </p>
                    </div>
                </div>
            @else
                <div class="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-[2rem] p-6 flex gap-5 items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-emerald-700 dark:text-emerald-300 mt-1">
                            Com as contribuições atuais terás <strong>{{ number_format($results['monthlyIncome'], 0, ',', '.') }}€/mês</strong> na reforma,
                            superando o teu objetivo de {{ number_format($targetMonthlyIncome, 0, ',', '.') }}€.
                            Excedente: <strong>+{{ number_format(abs($results['gap']), 0, ',', '.') }}€/mês</strong>.
                        </p>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] p-8 shadow-sm space-y-4">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between items-center text-sm">
                        ...
                        <span class="font-black dark:text-white">{{ number_format($results['totalContributed'], 0, ',', '.') }}€ ({{ round($pctContrib) }}%)</span>
                    </div>
                    <div class="h-3 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <div class="h-full bg-indigo-500 rounded-full transition-all duration-700" style="width: {{ $pctContrib }}%"></div>
                    </div>
                    <div class="flex justify-between items-center text-sm mt-2">
                        <span class="font-bold dark:text-white flex items-center gap-2">
                            <span class="size-3 rounded-full bg-emerald-500 inline-block"></span> Juros Compostos
                        </span>
                        <span class="font-black text-emerald-600">+{{ number_format(max(0, $results['totalGrowth']), 0, ',', '.') }}€ ({{ round($pctGrowth) }}%)</span>
                    </div>
                    <div class="h-3 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                        <div class="h-full bg-emerald-500 rounded-full transition-all duration-700" style="width: {{ $pctGrowth }}%"></div>
                    </div>
                </div>

                <p class="text-[9px] text-zinc-400 font-medium italic pt-2">
                    * Simulação baseada em retorno anual composto de {{ $annualReturn }}% e inflação de {{ $inflationRate }}%. Não constitui aconselhamento financeiro.
                </p>
            </div>
        </div>
    </div>
