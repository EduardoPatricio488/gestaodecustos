<div class="space-y-10 pb-20 text-left" x-data="{ showEditor: false, isExpanded: false }">
            ...
            <div class="flex items-center gap-6">
                <div class="relative group">
                    ...
                    <div class="relative p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] shadow-2xl">
                        <flux:icon name="user-plus" class="w-10 h-10 text-brand-600" />
                    </div>
                </div>
                <div class="text-left">
    ...
    {{-- 3. CENTRO DE COMANDO: CONTROLO DA MONTRA PÚBLICA --}}
    <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-center gap-8 shrink-0">
                ...
                <div class="flex flex-col items-center md:items-end gap-2 text-left">
                    ...
                    <div class="flex items-center gap-1 bg-zinc-50 dark:bg-zinc-950 p-1 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-inner">
                        <button type="button" wire:click="$set('recVacancies', {{ max(1, $recVacancies - 1) }})" class="size-8 flex items-center justify-center rounded-xl bg-white dark:bg-zinc-800 text-zinc-400 hover:text-red-500 transition-all shadow-sm border border-zinc-100 dark:border-zinc-700">-</button>
                        <div class="w-12 text-center"><span class="text-xl font-black text-zinc-900 dark:text-white tabular-nums italic">{{ sprintf('%02d', $recVacancies) }}</span></div>
                        <button type="button" wire:click="$set('recVacancies', {{ $recVacancies + 1 }})" class="size-8 flex items-center justify-center rounded-xl bg-white dark:bg-zinc-800 text-zinc-400 hover:text-emerald-500 transition-all shadow-sm border border-zinc-100 dark:border-zinc-700">+</button>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-center md:items-end gap-2 text-left">
                    ...
                    <button type="button" wire:click="toggleActive" class="flex items-center gap-4 bg-zinc-50 dark:bg-zinc-950 p-3 rounded-2xl border border-zinc-100 dark:border-zinc-800 hover:border-brand-500/50 transition-all group/toggle">
                        <span class="text-xs font-black uppercase {{ $recActive ? 'text-emerald-500' : 'text-red-500' }}">{{ $recActive ? 'Abertas' : 'Fechadas' }}</span>
                        <div class="pointer-events-none"><flux:checkbox wire:model="recActive" variant="toggle" color="emerald" class="scale-110" /></div>
                    </button>
                </div>
                ...
                <button @click="showEditor = !showEditor" class="group flex flex-col items-center gap-1">
                    <div class="size-12 rounded-full bg-zinc-950 text-white flex items-center justify-center hover:bg-brand-600 transition-all shadow-xl active:scale-90">
                        <flux:icon name="chevron-down" class="size-5 transition-transform duration-500" x-bind:class="showEditor ? 'rotate-180' : ''" />
                    </div>
...
<div class="absolute -top-6 right-10 md:right-20 z-30 group/announce">
    ...
    <div class="bg-zinc-950 border-2 border-brand-500 p-4 rounded-3xl shadow-[0_10px_30px_rgba(16,185,129,0.3)] flex items-start gap-3 w-64 md:w-80 transition-all duration-300">
        ...
        <flux:icon name="megaphone" variant="solid" class="size-5 text-brand-500 shrink-0 mt-1 animate-bounce" />
                    ...
                    <div class="p-12 md:p-20 space-y-12 text-left">
                        <div class="flex justify-between items-start">
                            <div class="size-20 rounded-3xl bg-zinc-50 dark:bg-zinc-950 flex items-center justify-center text-brand-600 font-black text-3xl border border-zinc-100 dark:border-zinc-800 shadow-inner">
                                 @if($workspace->logo_path) <img src="{{ asset('storage/' . $workspace->logo_path) }}" class="w-full h-full object-cover rounded-3xl">
                                 @else {{ substr($workspace->name, 0, 1) }} @endif
                            </div>
                            <span class="px-4 py-1.5 {{ $recActive ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/20' : 'bg-red-500/10 text-red-600 border-red-500/20' }} text-[9px] font-black uppercase rounded-xl border tracking-widest transition-all">
                                {{ $recActive ? 'Vagas Abertas ('.$recVacancies.')' : 'Candidaturas Fechadas' }}
                            </span>
                        </div>
                        ...
                        <div class="pt-6 flex justify-center border-t border-zinc-50 dark:border-zinc-800/50">
                            <button @click="isExpanded = !isExpanded" type="button" class="flex flex-col items-center gap-3 group/btn">
                                ...
                                <flux:icon name="chevron-down" class="size-6 text-zinc-300 group-hover/btn:text-brand-500 transition-all" x-bind:class="isExpanded ? 'rotate-180' : ''" />
                            </button>
                        </div>
                    </div>

                    {{-- ÁREA EXPANSÍVEL (DADOS EXTRA) --}}
                        ...
                        >
                            ...
                            <span wire:loading.remove wire:target="saveSettings" class="flex items-center gap-3">
                                @if(session()->has('published'))
                                    <flux:icon name="check-badge" variant="solid" class="size-6 text-emerald-400" />
                                    <span>Conteúdo Publicado</span>
                                @else
                                    <flux:icon name="check-circle" variant="solid" class="size-6" />
    ...
    {{-- 4. LISTA DE CANDIDATOS RECEBIDOS (PENDENTES + RECUSADOS) --}}
    <div class="space-y-6">
        <h3 class="text-sm font-black uppercase tracking-widest text-zinc-400 px-2 flex items-center gap-2 text-left">
            <flux:icon name="user-group" class="size-4" /> Candidaturas Recebidas
        </h3>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @forelse($applications as $candidate)
                @php $isRejected = $candidate->status === 'rejected'; @endphp

                <div wire:key="candidate-{{ $candidate->id }}"
                     class="bg-white dark:bg-zinc-900 border {{ $isRejected ? 'border-red-500/40 shadow-inner' : 'border-zinc-200 dark:border-zinc-800' }} rounded-[3rem] p-8 shadow-sm flex flex-col group transition-all hover:border-brand-500/30 text-left relative overflow-hidden">

                    {{-- SELO DE ESTADO CRÍTICO (SÓ APARECE SE RECUSADO) --}}
                    @if($isRejected)
                    ...
                    {{-- CONTEÚDO DO CARD (Fica com opacidade baixa se recusado) --}}
                    <div class="flex flex-col h-full {{ $isRejected ? 'opacity-40 grayscale-[50%]' : '' }}">
                        ...
                        <div class="flex justify-between items-start mb-6 text-left">
                            ...
                            <div class="text-right leading-none">
                                 <span class="text-[8px] font-black uppercase text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-3 py-1 rounded-lg">
                                    {{ $isRejected ? 'Histórico' : 'Pendente' }}
                                 </span>
                                 <p class="text-[8px] font-bold text-zinc-400 uppercase mt-2 italic">{{ \Carbon\Carbon::parse($candidate->created_at)->diffForHumans() }}</p>
                            </div>
                        </div>

                        <div class="space-y-4 flex-1 text-left">
                            <div>
                                <h4 class="font-black dark:text-white uppercase text-lg leading-tight">{{ $candidate->name }}</h4>
                                <p class="text-[10px] font-black text-brand-600 uppercase tracking-widest mt-1 italic">{{ $candidate->role }}</p>
                            </div>
                        ...
                        <div class="mt-8 pt-6 border-t border-zinc-100 dark:border-zinc-800 space-y-3 relative z-20">
                            ...
                            <div class="flex gap-2">
                                @if($isRejected)
                                    {{-- BOTÃO PARA REABILITAR --}}
                                    <button wire:click="reopenCandidate({{ $candidate->id }})"
                                ...
                                @else
                                    <button wire:click="rejectCandidate({{ $candidate->id }})" class="flex-1 py-3 border border-zinc-200 dark:border-zinc-800 text-zinc-400 dark:text-zinc-500 rounded-xl text-[9px] font-black uppercase hover:bg-red-50 hover:text-red-500 transition-all">Recusar</button>
                                    <button wire:click="acceptCandidate({{ $candidate->id }})" class="flex-[2] py-3 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg shadow-emerald-500/20 active:scale-95 transition-all">Contratar</button>
                                @endif
                            </div>
                        </div>
                    </div>
            ...
            @empty
                <div class="col-span-full py-24 text-center opacity-30 flex flex-col items-center">
                    <flux:icon name="user-group" class="size-16 mb-4" />
