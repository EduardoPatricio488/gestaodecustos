<div class="space-y-10 pb-20"> {{-- Única Raiz --}}
        ...
        <div class="flex justify-center mb-8">
            <div x-data="{ open: false }" class="relative">
                ...
                <button @click="open = !open" @click.outside="open = false" type="button"
                    ...
                    <div class="absolute -inset-0.5 bg-gradient-to-r {{ $isCollab ? 'from-emerald-500/40 to-teal-500/40' : 'from-brand-500/40 to-indigo-500/40' }} rounded-full blur opacity-20 group-hover:opacity-100 transition duration-500"></div>
                    ...
                    <div class="relative flex items-center gap-3 px-4 py-1.5 rounded-full bg-white/80 dark:bg-zinc-900/80 border border-zinc-200/50 dark:border-zinc-800/50 shadow-sm backdrop-blur-md">
                        ...
                        {{-- PONTO DE STATUS ÚNICO (Antes do IF para garantir que há apenas 1) --}}
                        <span class="relative flex h-2 w-2">
                            <span class="animate-ping absolute inline-flex h-full w-full rounded-full {{ $isCollab ? 'bg-emerald-400' : 'bg-brand-400' }} opacity-75"></span>
                            <span class="relative inline-flex rounded-full h-2 w-2 {{ $isCollab ? 'bg-emerald-500' : 'bg-brand-500' }}"></span>
                        </span>
                        ...
                        <flux:icon name="chevron-down" variant="micro" class="size-3 text-zinc-400 transition-transform duration-300" x-bind:class="open ? 'rotate-180' : ''" />
                    </div>
                </button>
                ...
                <div x-show="open" x-cloak x-transition
                    ...
                    <div class="space-y-1">
                        @forelse($workspace->employees as $employee)
                            <button wire:click="switchToEmployee({{ $employee->id }})" @click="open = false"
                                ...
                                <flux:icon name="user" variant="micro" class="size-4 text-zinc-400 group-hover:text-brand-500" />
                                <div class="flex flex-col leading-tight">
                                    <span class="font-black text-[10px] uppercase tracking-widest text-zinc-800 dark:text-white">{{ $employee->name }}</span>
                                    <span class="text-[9px] text-zinc-500 font-bold uppercase tracking-tighter">{{ $employee->role ?? 'Colaborador' }}</span>
                                </div>
                            </button>
                        @empty
 ...
 <img src="{{ $workspace->logo_url }}?t={{ time() }}" class="size-24 rounded-[2.5rem] shadow-2xl border-4 border-white dark:border-zinc-800 object-cover bg-white">                    <div class="absolute -bottom-1 -right-1 size-7 bg-emerald-500 border-4 border-zinc-50 dark:border-zinc-950 rounded-full shadow-lg"></div>
                ...
                <div>
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
                                    ...
                                    <div class="space-y-1 max-h-64 overflow-y-auto">
                                        @foreach($businessWorkspaces as $bws)
                                            <button
                                                wire:click="switchBusinessWorkspace({{ $bws->id }})"
                                                @click="open = false"
                                                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-left group/item {{ $bws->id === $workspace->id ? 'bg-brand-50 dark:bg-brand-950/30' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800' }}"
                                            >
                                                <img src="{{ $bws->logo_url }}" class="size-8 rounded-xl object-cover bg-zinc-100 shrink-0" />
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <p class="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">
                                                        @if($bws->user_role === 'admin') CEO / Proprietário
                                                        @elseif($bws->user_role === 'editor') Colaborador
                                                        @else Visitante
                                                        @endif
                                                    </p>
                                                </div>
                                                @if($bws->id === $workspace->id)
                    ...
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 mt-3">
                        <p class="text-xs font-black text-zinc-400 uppercase tracking-widest flex items-center gap-2">
                            <flux:icon name="identification" class="size-3" /> NIF: <span :class="privacyMode ? 'blur-sm select-none' : ''">{{ ($workspace->tax_number ?? '') !== '' ? implode(' ', str_split(preg_replace('/\D/', '', (string) $workspace->tax_number), 3)) : 'S/ NIF' }}</span>
                        </p>
                        ...
                        <p class="text-xs font-black text-brand-600 dark:text-brand-400 uppercase tracking-widest flex items-center gap-2">
                            <flux:icon name="building-office" class="size-3" /> {{ $workspace->industry ?? 'Gestão de Negócio' }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 bg-white dark:bg-zinc-900 p-2.5 rounded-[1.8rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <flux:button href="{{ route('export.business') }}" variant="ghost" icon="document-arrow-down" class="rounded-2xl font-bold text-zinc-500">Contabilista</flux:button>
                <div class="h-6 w-px bg-zinc-200 dark:bg-zinc-800 mx-1"></div>
                <flux:button href="{{ route('hub.business.ai') }}" variant="primary" icon="sparkles" class="bg-brand-600 border-none shadow-lg rounded-2xl font-black uppercase tracking-tighter px-6 text-white">Estrategista IA</flux:button>
            </div>
        </div>
    </div>
    ...
    {{-- 2. GRID DE PERFORMANCE FINANCEIRA --}}
    ...
    {{-- 3. BENTO GRID — PROJETOS & RISCO IA --}}
        ...
        <div class="lg:col-span-2 glass-card p-8 bg-white dark:bg-zinc-900 rounded-[2.5rem]
            ...
            <div class="flex justify-between items-center mb-10">
                ...
                <flux:badge variant="neutral"
                    class="font-black text-[9px] uppercase bg-zinc-100 dark:bg-zinc-800 border-none px-3 py-1">
                    ...
                    <div class="p-6 bg-zinc-50/50 dark:bg-zinc-800/40 rounded-[2rem]
                        ...
                        <div class="h-2 w-full bg-zinc-200 dark:bg-zinc-700 rounded-full overflow-hidden
                            ...
                            <div class="h-full bg-brand-500 transition-all duration-1000
                                ...
                                style="width: {{ min(100, max(0, $project->margin)) }}%">
                            </div>
                        </div>
                    </div>
                @empty
                ...
                <div class="space-y-6 flex-1">
                    ...
                    <div class="flex items-center justify-between p-4 bg-white/5 rounded-2xl
                        ...
                        <span :class="privacyMode ? 'blur-sm select-none' : ''"
                            ...
                            {{ $lowStockCount > 0 ? 'bg-red-500 text-white' : 'bg-emerald-500/10 text-emerald-500' }}
                    ...
                    <div class="flex items-center justify-between p-4 bg-white/5 rounded-2xl
                        ...
                        <span :class="privacyMode ? 'blur-sm select-none' : ''"
                            class="text-[10px] font-black px-3 py-1 rounded-lg
                            {{ $criticalDocsCount > 0 ? 'bg-amber-500 text-white' : 'bg-emerald-500/10 text-emerald-500' }}
                            transition-all duration-500">
                    ...
                    <div class="flex items-center justify-between p-4 bg-white/5 rounded-2xl
                        ...
                        <span :class="privacyMode ? 'blur-sm select-none' : ''"
                            class="text-[10px] font-black px-3 py-1 rounded-lg
                            {{ $overdueTasksCount > 0 ? 'bg-red-500 text-white shadow-[0_0_10px_rgba(239,68,68,0.3)]' : 'bg-emerald-500/10 text-emerald-500' }}
                            transition-all duration-500">
        ...
        <div class="glass-card p-8 bg-white dark:bg-zinc-900 rounded-[2.5rem]
            ...
            <div class="flex justify-between items-start">
                ...
                <flux:badge variant="neutral" size="sm"
                    class="font-black text-[9px] tracking-widest bg-zinc-100 dark:bg-zinc-800 border-none px-3 py-1">
                    <span :class="privacyMode ? 'blur-sm select-none' : ''"
                        class="transition-all duration-500">
                        {{ $teamCount }} PESSOAS
                    </span>
