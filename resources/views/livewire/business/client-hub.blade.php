        ...
        <div wire:key="client-card-{{ $client->id }}"
            ...
            <div class="p-7 flex justify-between items-start">
                <div class="flex items-center gap-5">
                    ...
                    <div class="text-left">
                        ...
                        <div class="flex items-center gap-2 mt-2">
                            ...
                            @endphp
                            <span class="inline-flex px-2 py-0.5 font-black uppercase text-[8px] tracking-widest rounded-md border {{ $statusStyle }}">{{ $client->status }}</span>
                            <span class="text-[10px] text-zinc-400 font-bold uppercase tracking-tighter">NIF: {{ $client->tax_number ? implode(' ', str_split(preg_replace('/\D/', '', (string) $client->tax_number), 3)) : '---' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative">
                    ...
                    <div x-show="optionsOpen" x-cloak @click.outside="optionsOpen = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 top-10 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl shadow-2xl z-[100] overflow-hidden text-left">
                        <div class="p-1.5 space-y-0.5">
                            <button type="button" wire:click="edit({{ $client->id }})" @click="optionsOpen = false" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest text-zinc-600 dark:text-zinc-300 hover:bg-brand-50 hover:text-brand-600 transition-all">
                                <flux:icon name="pencil-square" class="size-4 text-brand-500" />
                                Editar Ficha
                            </button>
                            <div class="border-t border-zinc-100 dark:border-zinc-800 my-1"></div>
                            <button type="button" wire:click="delete({{ $client->id }})" @click="optionsOpen = false" wire:confirm="Apagar cliente e todo o histórico associado?" class="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-[11px] font-black uppercase tracking-widest text-red-500 hover:bg-red-50 transition-all">
            ...
            <div class="grid grid-cols-2 gap-px bg-zinc-100 dark:bg-zinc-800 border-y border-zinc-100 dark:border-zinc-800">
                <div class="p-6 bg-white dark:bg-zinc-900 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-950/50 transition-colors text-left">
                    ...
                    <p class="text-xl font-black text-emerald-600 tracking-tighter italic leading-none">
                        <span :class="privacyMode ? 'blur-md select-none' : ''" class="transition-all duration-500 inline-block">{{ number_format($client->total_revenue, 2, ',', ' ') }} €</span>
                    </p>
                </div>
                <div class="p-6 bg-white dark:bg-zinc-900 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-950/50 transition-colors border-l border-zinc-100 dark:border-zinc-800 text-left">
                    ...
                    <p class="text-xl font-black {{ $client->pending_debt > 0 ? 'text-red-500' : 'text-zinc-300' }} tracking-tighter italic leading-none">
                        <span :class="privacyMode ? 'blur-md select-none' : ''" class="transition-all duration-500 inline-block">{{ number_format($client->pending_debt, 2, ',', ' ') }} €</span>
                    </p>
                </div>
            </div>
        ...
        <div class="relative max-h-[90vh] overflow-y-auto overscroll-contain p-10 bg-white dark:bg-zinc-950 rounded-[2.5rem] space-y-10 shadow-2xl border border-zinc-200 dark:border-zinc-800">
            <div class="flex items-center justify-between gap-4">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-brand-50 dark:bg-brand-500/10 rounded-2xl text-brand-600">
            ...
            <div class="space-y-8">
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <flux:input wire:model="name" placeholder="Ex: Acme Portugal" class="font-bold !bg-zinc-50 dark:!bg-zinc-900 !border-none rounded-2xl h-14 shadow-inner" />
                        </div>
                        <div class="space-y-2">
                            <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Nome Fiscal (Legal)</flux:label>
                            <flux:input wire:model="legal_name" placeholder="Ex: Acme Corp, Lda" class="font-bold !bg-zinc-50 dark:!bg-zinc-900 !border-none rounded-2xl h-14 shadow-inner" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <flux:input
                            ...
                            maxlength="11"
                            autocomplete="off"
                            class="font-black !bg-zinc-50 dark:!bg-zinc-900 !border-none rounded-2xl h-14 shadow-inner tracking-wider"
                        />
                        <p class="px-1 text-[9px] font-bold text-zinc-400 uppercase tracking-widest">O NIF é dividido automaticamente em grupos de 3 dígitos.</p>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <flux:textarea wire:model="notes" rows="2" placeholder="Regista aqui detalhes sobre o cliente, acordos feitos ou preferências..." class="rounded-2xl shadow-sm border-none !bg-zinc-50 dark:!bg-zinc-900 text-sm p-4" />
                </div>
            </div>
...
<flux:modal name="portal-link-modal" position="center" class="!fixed !inset-0 !m-auto w-[calc(100vw-2rem)] max-w-[500px] !max-h-[90vh] !p-0 overflow-hidden">
        ...
        <div class="relative max-h-[90vh] overflow-y-auto overscroll-contain p-10 bg-white dark:bg-zinc-950 rounded-[2.5rem] space-y-8 shadow-2xl border border-zinc-200 dark:border-zinc-800 text-left">
            @if($selectedClient)
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div class="flex items-center gap-4">
                        <div class="p-3 bg-zinc-100 dark:bg-zinc-800 rounded-2xl"><flux:icon name="clock" class="size-6 text-zinc-500" /></div>
                        <div>
                        ...
                        <div class="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
                            @forelse($selectedClient->invoices as $invoice)
                                <div class="p-4 bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-2xl flex justify-between items-center shadow-sm">
                                    ...
                                    <p class="text-sm font-black {{ $invoice->status === 'paga' ? 'text-emerald-500' : 'text-amber-500' }}">{{ number_format($invoice->amount, 2, ',', ' ') }}€</p>
                                </div>
                            @empty
    ...
    <footer class="pt-20 pb-6 text-center border-t border-zinc-100 dark:border-zinc-800 mt-20">
        <p class="text-[9px] font-black text-zinc-400 uppercase tracking-[0.4em]">© {{ date('Y') }} {{ config('app.name') }} · Protocolo de Gestão de Relacionamento</p>
    </footer>
</div>
