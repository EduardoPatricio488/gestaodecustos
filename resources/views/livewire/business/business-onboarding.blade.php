                ...
                <div class="grid gap-6">
                    ...
                    @if($industry === 'Outro')
                        <div class="animate-in slide-in-from-top-2 duration-300">
                            <flux:input wire:model="customIndustry" label="Especifique a sua Área" placeholder="Ex: Aviação, Moda, Cinema..." class="bg-brand-500/5 border-brand-500/20" />
                        </div>
                    @endif
                    ...
                    <flux:input
                        ...
                        icon="envelope"
                    />
                    <p class="-mt-4 text-[10px] font-bold text-zinc-400">Obrigatório. Este endereço recebe pedidos de acesso bancário e outras comunicações empresariais.</p>
                </div>
        ...
        @if($step == 3)
            <div class="space-y-8 animate-in slide-in-from-right duration-300 text-center"
                 x-data="{
                    ...
                    cleanValue(val) { return val.replace(/\s/g, ''); }
                 }">
                ...
                <div class="relative max-w-xs mx-auto my-10">
                    ...
                    <input type="text" inputmode="numeric" x-ref="moneyInput" x-init="$el.value = formatValue($wire.initial_capital)"
                           x-on:input="let val = $event.target.value; $el.value = formatValue(val); $wire.set('initial_capital', cleanValue($el.value));"
                           placeholder="0"
