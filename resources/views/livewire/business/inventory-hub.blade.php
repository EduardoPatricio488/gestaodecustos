<div class="space-y-10 pb-20">
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 relative z-10">
            <div class="flex items-center gap-6">
                ...
                <div>
                    <div class="flex items-center gap-3">
                        <h1 class="text-4xl font-black dark:text-white uppercase tracking-tighter italic leading-none text-zinc-900 dark:text-white">Stock & Inventário</h1>
                        <flux:badge variant="neutral" class="bg-zinc-100 dark:bg-zinc-800 text-[9px] font-black uppercase tracking-widest border-none px-3 py-1 text-zinc-500">Logística Ativa</flux:badge>
                    </div>
                    <p class="text-sm text-zinc-500 font-medium italic mt-2 text-zinc-400">Controlo de mercadorias, valor de armazém e reposição</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 bg-white dark:bg-zinc-900 p-2.5 rounded-[1.8rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <div class="relative flex-1 min-w-[240px]">
                    <flux:input wire:model.live="search" icon="magnifying-glass" placeholder="Procurar artigo..." class="!bg-transparent border-none shadow-none" />
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="relative overflow-hidden bg-zinc-950 p-8 rounded-[2.5rem] shadow-2xl border border-zinc-800 group">
            ...
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-4">
                    ...
                    <span class="text-[9px] font-black text-white/50 border border-white/10 px-2 py-1 rounded-lg uppercase tracking-widest italic text-zinc-400">Ativos Brutos</span>
                </div>
                <p class="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-1">Valor Estimado de Venda</p>
        ...
        {{-- Alertas de Stock --}}
        <div class="glass-card relative overflow-hidden bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm group transition-all {{ $lowStockCount > 0 ? 'hover:border-red-500/30' : 'hover:border-emerald-500/30' }}">
            <div class="flex justify-between items-start mb-4">
                <div class="p-3 {{ $lowStockCount > 0 ? 'bg-red-50 dark:bg-red-500/10 text-red-600' : 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600' }} rounded-2xl text-red-600">
                    <flux:icon name="{{ $lowStockCount > 0 ? 'exclamation-triangle' : 'check-badge' }}" variant="outline" class="size-6" />
                </div>
            </div>
            <p class="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-1">Reposição Necessária</p>
            <h3 class="text-4xl font-black {{ $lowStockCount > 0 ? 'text-red-500' : 'text-zinc-900 dark:text-white' }} tracking-tighter">
                {{-- BLUR NO NÚMERO DE ARTIGOS --}}
                <span :class="privacyMode ? 'blur-sm select-none' : ''" class="transition-all duration-500 inline-block">
                    {{ $lowStockCount }}
                </span>
                <span class="text-xs text-zinc-400 uppercase font-bold ml-2 tracking-widest italic">Artigos</span>
            </h3>
        </div>

        {{-- Margem Média --}}
        <div class="glass-card relative overflow-hidden bg-white dark:bg-zinc-900 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-800 shadow-sm transition-all hover:border-emerald-500/30">
            <div class="flex justify-between items-start mb-4">
            ...
            <p class="text-[10px] font-black text-zinc-400 uppercase tracking-[0.2em] mb-1">Margem Média Bruta</p>
            ...
            <div class="mt-4 h-1.5 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                <div class="h-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]" style="width: {{ $margin }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="glass-card bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] shadow-sm overflow-hidden group">
        <div class="p-8 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/30 dark:bg-zinc-900/30">
            <div>
                <h2 class="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400 mb-1">Mapa de Mercadorias</h2>
                <p class="text-lg font-black dark:text-white uppercase italic tracking-tighter text-zinc-800 dark:text-zinc-200">Custódia de Artigos em Armazém</p>
            </div>
            <div class="flex items-center gap-3">
                <flux:badge variant="neutral" class="bg-zinc-100 dark:bg-zinc-800 text-[10px] font-black uppercase px-3 py-1 border-none shadow-sm">{{ $products->count() }} Referências</flux:badge>
            </div>
        </div>
                        ...
                        <tr class="hover:bg-zinc-50 dark:hover:bg-brand-500/5 transition-all duration-300 group/row">
                            ...
                            <td class="p-6">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div class="flex flex-col">
                                        <span class="text-sm font-black dark:text-white uppercase tracking-tight">{{ $product->name }}</span>
                                        <span class="text-[10px] text-zinc-500 font-bold uppercase mt-0.5">{{ $product->sku ?? 'S/ SKU' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="p-6 text-center">
                                <div class="inline-flex flex-col items-center">
                                    <span class="inline-flex px-3 py-1 font-black uppercase text-[9px] tracking-widest rounded-xl border transition-all duration-500 {{ $isLowStock ? 'bg-red-100 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800' : 'bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800' }}">
                            ...
                            <td class="p-6 text-right">
                                <span :class="privacyMode ? 'blur-sm select-none' : ''" class="text-xs font-bold text-zinc-500 transition-all duration-500">
                            ...
                            <td class="p-6 text-right">
                                <span :class="privacyMode ? 'blur-sm select-none' : ''" class="text-sm font-bold dark:text-zinc-200 transition-all duration-500">
                            ...
                            <td class="p-6 text-right px-10">
                                <div class="flex flex-col items-end">
                                    ...
                                    <span class="text-[8px] font-black text-zinc-400 uppercase opacity-70">Potencial de Venda</span>
                                </div>
                            </td>

                            {{-- AÇÕES --}}
                            <td class="p-6 text-right pr-8">
                                <flux:button wire:click="edit({{ $product->id }})" variant="ghost" icon="pencil-square" size="sm" class="opacity-0 group-hover/row:opacity-100 transition-opacity rounded-xl" />
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="6" class="p-24 text-center text-zinc-400 italic font-medium">Sem artigos registados.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="relative p-10 bg-white dark:bg-zinc-950 rounded-[2.5rem] space-y-10 shadow-2xl border border-zinc-200 dark:border-zinc-800">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-zinc-400 font-medium">Configura os parâmetros de inventário e margens comerciais.</p>
                </div>
            </div>
            ...
            <div class="space-y-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-2 space-y-2">
                        <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Designação do Produto</flux:label>
                        <flux:input
                            wire:model="name"
                    ...
                    <div class="space-y-2">
                        <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">SKU / Referência</flux:label>
                        <flux:input
                            wire:model="sku"
                            placeholder="REF-001"
                ...
                <div class="p-6 bg-zinc-50 dark:bg-zinc-900/50 rounded-[2rem] border border-zinc-100 dark:border-zinc-800 space-y-6 shadow-inner">
                    <div class="flex items-center gap-2 px-1">
                        ...
                        <p class="text-[9px] font-black uppercase text-brand-600 tracking-[0.2em]">Parâmetros de Reposição</p>
                    </div>

                    <div class="grid grid-cols-2 gap-6">
                        <div class="space-y-2 text-center">
                            <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Unidades em Stock</flux:label>
                            <flux:input
                        ...
                        <div class="space-y-2 text-center">
                            <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Ponto de Alerta (Mín.)</flux:label>
                            <flux:input
                ...
                <div class="grid grid-cols-2 gap-6">
                    <div class="space-y-2">
                        <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Custo de Compra (€)</flux:label>
                        <flux:input
                    ...
                    <div class="space-y-2">
                        <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Preço de Venda (€)</flux:label>
                        <flux:input
