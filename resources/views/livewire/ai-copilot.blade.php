            ...
            <header class="flex items-center justify-between border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
                <div class="min-w-0">
                    <div class="flex items-center gap-2">
                        <span class="flex h-8 w-8 items-center justify-center rounded-xl bg-zinc-950 text-white dark:bg-white dark:text-zinc-950">✦</span>
                        <div>
                            <h2 class="text-sm font-black tracking-tight text-zinc-950 dark:text-white">Finance Pro AI</h2>
                            <p class="text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-400">Financial Copilot</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-1">
                    ...
                    <button wire:click="toggle" type="button" class="rounded-lg px-2 py-1.5 text-lg text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-900">×</button>
                </div>
            </header>

            <div class="border-b border-zinc-100 bg-zinc-50 px-5 py-3 dark:border-zinc-900 dark:bg-zinc-900/60">
            ...
            <div class="flex-1 space-y-4 overflow-y-auto p-4" id="finance-pro-ai-messages">
                @forelse($messages as $message)
                    <div class="flex {{ $message['role'] === 'user' ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[88%] rounded-2xl px-4 py-3 {{ $message['role'] === 'user' ? 'bg-zinc-950 text-white dark:bg-white dark:text-zinc-950' : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-100' }}">
                ...
                @empty
                    <div class="flex h-full min-h-64 flex-col items-center justify-center px-8 text-center">
                        ...
                        <p class="mt-2 text-xs leading-5 text-zinc-500">Pergunta-me sobre as tuas despesas, objetivos, investimentos ou, no workspace empresarial, sobre a performance da empresa.</p>
                    </div>
                @endforelse
            ...
            @if($pendingActions)
                <div class="max-h-56 space-y-3 overflow-y-auto border-t border-amber-200 bg-amber-50 p-4 dark:border-amber-900/50 dark:bg-amber-950/20">
                    @foreach($pendingActions as $action)
                        <div class="rounded-2xl border border-amber-200 bg-white p-4 dark:border-amber-900/50 dark:bg-zinc-950">
                            ...
                            <div class="mt-3 space-y-1 text-[11px] text-zinc-600 dark:text-zinc-300">
                                @foreach(($action['details'] ?? []) as $key => $value)
                                    @if(is_scalar($value))
                                        <div class="flex justify-between gap-3"><span class="font-bold">{{ $key }}</span><span class="text-right">{{ $value }}</span></div>
                                    @endif
                                @endforeach
                            </div>
