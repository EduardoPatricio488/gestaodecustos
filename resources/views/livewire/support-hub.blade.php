<div class="space-y-10 pb-24"> {{-- ÚNICO ELEMENTO RAIZ --}}

    {{-- 1. HEADER DINÂMICO (Design Premium Verde) --}}
    <div class="relative">
        {{-- Glow de fundo sempre Verde (Brand) --}}
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 relative z-10 px-2">
            <div class="flex items-center gap-6">
                <div class="relative group">
                    ...
                    <div class="relative p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] shadow-2xl">
                        {{-- Ícone muda conforme o contexto, mas a cor é sempre a Brand (Verde) --}}
            ...
            <div class="flex items-center gap-3 bg-white dark:bg-zinc-900 p-2.5 rounded-[1.8rem] border border-zinc-200 dark:border-zinc-800 shadow-sm">
                <flux:modal.trigger name="new-ticket-modal">
                    <flux:button variant="primary" icon="plus" class="rounded-2xl px-6 font-black uppercase tracking-widest shadow-lg bg-brand-600 shadow-brand-500/20 border-none text-white">
                        Novo Ticket
                    </flux:button>
                </flux:modal.trigger>
                <div class="h-6 w-px bg-zinc-200 dark:bg-zinc-800 mx-1"></div>
                {{-- Volta para o Dashboard correto --}}
                <flux:button href="{{ $isBusinessMode ? route('hub.business.dashboard') : route('dashboard') }}"
                             variant="ghost" icon="arrow-left" wire:navigate title="Voltar" class="rounded-xl" />
            </div>
        </header>
    </div>
    ...
    {{-- 3. LEDGER DE ASSISTÊNCIA (Lista de Tickets) --}}
    <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-5 flex-1 w-full">
                            ...
                            <div class="space-y-1">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-[10px] text-zinc-500 font-bold uppercase italic">{{ $ticket->messages_count }} interações</span>
                                </div>
                            </div>
                        </div>
    ...
    Entrar no Chat
                                ...
                                @if($isOpen)
                                    <flux:button wire:click="closeTicket({{ $ticket->id }})" wire:confirm="Confirmas que o problema foi resolvido?" variant="ghost" icon="check-circle" color="emerald" class="rounded-xl" title="Fechar Caso" />
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    {{-- 4. MODAL DE CONVERSA (ESTILO MESSENGER PREMIUM) --}}
    <flux:modal name="ticket-chat-modal" position="center" class="md:w-[600px] !p-0 overflow-visible">
        @if($activeTicket)
            <div class="relative p-10 bg-white dark:bg-zinc-950 rounded-[2.5rem] space-y-8 flex flex-col h-[700px] shadow-2xl border border-zinc-200 dark:border-zinc-800">
                ...
                <div class="absolute top-6 right-6">
                    <flux:modal.close>
                        <flux:button variant="ghost" size="sm" icon="x-mark" class="rounded-full hover:bg-zinc-100 dark:hover:bg-zinc-800" />
                    </flux:modal.close>
                </div>
                    ...
                    <div class="pt-4 border-t dark:border-zinc-800">
                        <form wire:submit.prevent="sendReply" class="relative group">
                            <div class="flex items-center gap-3 p-2 bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-inner focus-within:ring-2 focus-within:ring-brand-500/20 transition-all">
                                ...
                                />
                                <flux:button type="submit" variant="primary" class="rounded-xl shadow-lg h-10 w-12 flex items-center justify-center bg-brand-600 border-none">
                ...
                @else
                    <div class="p-4 bg-zinc-100 dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 text-center">
                        <p class="text-[10px] font-black uppercase text-zinc-500 tracking-widest italic">Este ticket foi resolvido e o canal de chat arquivado.</p>
                    </div>
                @endif
            </div>
        @endif
    ...
    {{-- 5. MODAL: NOVO PEDIDO (DESIGN SaaS PRO) --}}
    <flux:modal name="new-ticket-modal" position="center" class="md:w-[500px] !p-0 overflow-visible">
        <div class="relative p-10 bg-white dark:bg-zinc-950 rounded-[2.5rem] space-y-8 shadow-2xl border border-zinc-200 dark:border-zinc-800">
            ...
            <form wire:submit="openTicket" class="space-y-6">
                <div class="space-y-2">
                    <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-1">Assunto do Pedido</flux:label>
                    <flux:input wire:model="subject" placeholder="Ex: Erro ao emitir fatura..." class="font-bold !bg-zinc-50 dark:!bg-zinc-900 !border-none rounded-2xl h-14 shadow-inner" />
                </div>

                <div class="space-y-2">
                    <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-1">Nível de Urgência</flux:label>
                ...
                <div class="space-y-2">
                    <flux:label class="text-[10px] font-black uppercase text-zinc-400 tracking-widest px-1">Descrição Detalhada</flux:label>
                    <flux:textarea wire:model="message" rows="3" placeholder="Forneça o máximo de detalhes possível..." class="rounded-2xl shadow-inner border-none !bg-zinc-50 dark:!bg-zinc-900 text-sm p-4" />
                </div>

                <div class="flex gap-4 pt-4">
                    <flux:modal.close><flux:button variant="ghost" class="flex-1 font-black uppercase text-[10px] text-zinc-400">Descartar</flux:button></flux:modal.close>
                    <flux:button type="submit" variant="primary" class="flex-[2] font-black uppercase tracking-widest shadow-xl shadow-brand-500/20 h-14 rounded-2xl bg-brand-600 border-none">
                        Enviar Pedido
                    </flux:button>
                </div>
            </form>
    ...
    <footer class="pt-20 pb-6 text-center border-t border-zinc-100 dark:border-zinc-800 mt-20 opacity-60">
        <p class="text-[9px] font-black text-zinc-400 uppercase tracking-[0.4em]">
            © {{ date('Y') }} {{ auth()->user()->currentWorkspace->name ?? config('app.name') }} · Suporte Técnico {{ $isBusinessMode ? 'Enterprise' : 'Pessoal' }}
        </p>
    </footer>

    {{-- 7. ESTILOS DE INTERFACE --}}
    <style>
...
</div> {{-- FECHO DA DIV RAIZ PRINCIPAL QUE ABRE NA PARTE 1 --}}
