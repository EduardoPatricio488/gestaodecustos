...
<div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 sm:p-10 rounded-[2.5rem] shadow-2xl backdrop-blur-md space-y-8 text-left">
    ...
    <form wire:submit.prevent="login" class="space-y-6">
        ...
        <div class="space-y-5">
            ...
            <div class="space-y-2">
                ...
                <input wire:model="token" type="text" maxlength="8" autocomplete="one-time-code" placeholder="Token de acesso"
                       class="w-full h-12 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-center font-mono font-black text-xl tracking-[0.45em] text-zinc-900 dark:text-white focus:ring-2 focus:ring-zinc-500 outline-none transition-all" />
            </div>
    ...
    <div class="pt-2 border-t border-zinc-100 dark:border-zinc-800/50">
        <button type="button" x-data x-on:click="$dispatch('open-bank-request')"
            ...
            Solicitar acesso como banco
        </button>
        <p class="text-center text-[9px] text-zinc-400 font-semibold mt-2">Pedido reservado a entidades bancárias com email institucional.</p>
    </div>
        ...
        <div x-show="open" x-transition @click.outside="open = false"
            ...
            <div class="px-7 py-6 border-b border-zinc-100 dark:border-zinc-800 flex items-start justify-between shrink-0">
                <div>
                    ...
                    <h2 class="mt-1 text-2xl font-black uppercase italic tracking-tighter text-zinc-900 dark:text-white">Solicitar credenciais</h2>
                    <p class="mt-1 text-xs text-zinc-500">Identifica o banco, seleciona a empresa e envia o pedido para o email empresarial registado.</p>
                </div>
            ...
            <div class="overflow-y-auto p-7 space-y-6">
                @if($requestSent)
                    <div class="rounded-2xl border border-emerald-200 bg-emerald-50 dark:border-emerald-500/20 dark:bg-emerald-500/10 p-5 text-center">
                        ...
                        <p class="mt-1 text-xs text-emerald-700/70 dark:text-emerald-300/70">A empresa foi notificada no email empresarial registado. Terá de validar o pedido antes de enviar as credenciais.</p>
                    </div>
                @endif

                <div class="space-y-2">
                    <label class="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Nome do banco</label>
                    <input wire:model="bankName" type="text" autocomplete="organization" placeholder="Ex: Banco Santander Totta"
                    ...
                    @error('bankName') <p class="text-xs text-red-500 font-semibold">{{ $message }}</p> @enderror
                </div>

                <div class="space-y-2">
                    <label class="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-400">Email institucional do banco</label>
                    <input wire:model="requestEmail" type="email" autocomplete="email" placeholder="nome@banco.pt"
                           class="w-full h-12 px-4 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl text-sm font-semibold outline-none focus:ring-2 focus:ring-brand-500 dark:text-white" />
                    @error('requestEmail') <p class="text-xs text-red-500 font-semibold">{{ $message }}</p> @enderror
                    <p class="text-[10px] text-zinc-400">Por segurança, endereços pessoais como Gmail, Outlook, Hotmail, Yahoo ou similares não são aceites.</p>
                </div>
                ...
                @if($selectedCompanyId)
                    ...
                    <div class="rounded-2xl bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 p-4">
                        ...
                        <p class="mt-1 text-[10px] text-zinc-500">O pedido será enviado para o email empresarial privado desta empresa.</p>
                    </div>
                @endif
                ...
                <button type="button" wire:click="sendAccessRequest" wire:loading.attr="disabled"
                    ...
                    <span wire:loading.remove wire:target="sendAccessRequest">Enviar pedido ao email da empresa</span>
