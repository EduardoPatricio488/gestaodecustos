<div class="max-w-6xl mx-auto py-12 px-6 space-y-12 text-left">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
        ...
        <div class="glass-card p-8 bg-zinc-950 text-white border border-zinc-800 rounded-[2.5rem] flex flex-col relative scale-105 shadow-2xl {{ $currentPlan == 'pro' ? 'ring-4 ring-violet-500/20' : '' }}">
            ...
            <flux:button wire:click="upgrade('pro')" variant="primary" color="violet" class="w-full !h-14 font-black uppercase tracking-widest bg-violet-600 hover:bg-violet-700 border-none shadow-xl shadow-violet-500/20" :disabled="$currentPlan == 'pro'">
    ...
    @if($showSuccessModal)
        <div class="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-zinc-950/80 backdrop-blur-xl animate-in fade-in duration-500">
            <div class="bg-white dark:bg-zinc-900 w-full max-w-md rounded-[3rem] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-300">
                ...
                <p class="text-zinc-500 mt-4">Parabéns, agora tens acesso a todas as vantagens do plano <span class="font-black text-{{ $newPlanData['color'] }}-500 uppercase">{{ $newPlanData['name'] }}</span>.</p>

                <div class="mt-8 p-6 bg-zinc-50 dark:bg-zinc-800/50 rounded-3xl border border-zinc-100 dark:border-zinc-800 text-left space-y-3">
                    <p class="text-[10px] font-black uppercase text-zinc-400">O que ganhaste:</p>
                    @if($newPlanData['raw'] === 'pro')
                        <p class="text-sm font-bold flex items-center gap-2"><flux:icon name="check" class="size-4 text-violet-500" /> Área Empresa Completa</p>
                        <p class="text-sm font-bold flex items-center gap-2"><flux:icon name="check" class="size-4 text-violet-500" /> Consultor IA Estratégico</p>
                    @else
                        <p class="text-sm font-bold flex items-center gap-2"><flux:icon name="check" class="size-4 text-emerald-500" /> Scanner IA de Recibos</p>
                        <p class="text-sm font-bold flex items-center gap-2"><flux:icon name="check" class="size-4 text-emerald-500" /> Contas de Casal/Família</p>
                    @endif
                </div>

                <flux:button wire:click="finish" variant="primary" color="{{ $newPlanData['color'] }}" class="w-full h-14 mt-10 rounded-2xl font-black uppercase tracking-widest">
                    Começar a usar
                </flux:button>
            </div>
        </div>
