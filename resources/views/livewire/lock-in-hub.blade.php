...
>
    ...
    <div class="flex-1 overflow-y-auto px-4 py-4 lg:px-6 lg:py-5">
        <div class="grid grid-cols-12 gap-4 max-w-[1600px] mx-auto">
            ...
            <div class="col-span-12 lg:col-span-4 flex flex-col gap-4">
                ...
                <div class="anim-2 glass rounded-2xl border p-4 glow-blue">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-[8px] text-zinc-600 uppercase tracking-widest">{{ now()->format('M Y') }}</span>
                    </div>

                    {{-- Saldo Consolidado --}}
                    <div class="mb-4">
                        <div class="text-[9px] text-zinc-500 uppercase tracking-widest mb-1">Saldo Consolidado</div>
                        <div class="tabular text-3xl font-black privacy-val {{ $personalBalance >= 0 ? 'text-white num-highlight' : 'text-red-400' }}">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-3">
                            <div class="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">Receitas {{ now()->format('M') }}</div>
                            <div class="tabular text-base font-black text-emerald-400 privacy-val">+{{ number_format($personalIncomeMonth, 2, ',', '.') }}€</div>
                        </div>
                        <div class="bg-orange-500/5 border border-orange-500/15 rounded-xl p-3">
                            <div class="text-[8px] text-zinc-500 uppercase tracking-widest mb-1">Gastos {{ now()->format('M') }}</div>
                            <div class="tabular text-base font-black text-orange-400 privacy-val">-{{ number_format($personalExpenseMonth, 2, ',', '.') }}€</div>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                        ...
                        <span class="tabular text-sm font-black privacy-val {{ $personalNetMonth >= 0 ? 'text-emerald-400' : 'text-red-400' }}">
                            {{ $personalNetMonth >= 0 ? '+' : '' }}{{ number_format($personalNetMonth, 2, ',', '.') }}€
                        </span>
                    </div>
                </div>
                ...
                <div class="anim-2 glass rounded-2xl border border-white/5 p-4">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="text-[10px] font-black text-white">{{ $goalProgress }}%</span>
                    </div>

                    <div class="text-sm font-bold text-zinc-300 mb-3 truncate">{{ $goalName }}</div>

                    {{-- Progress Bar --}}
                    <div class="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-white/5 mb-2">
                        <div
                            class="h-full rounded-full transition-all duration-700 {{ $goalProgress >= 80 ? 'bg-gradient-to-r from-emerald-500 to-emerald-300' : ($goalProgress >= 40 ? 'bg-gradient-to-r from-amber-500 to-amber-300' : 'bg-gradient-to-r from-red-600 to-red-400') }}"
                            style="width: {{ $goalProgress }}%"
                        ></div>
                    </div>

                    <div class="flex justify-between text-[9px] text-zinc-500">
                        <span class="privacy-val">{{ number_format($goalCurrent, 2, ',', '.') }}€ atual</span>
                        <span class="privacy-val">Meta: {{ number_format($goalTarget, 2, ',', '.') }}€</span>
                    </div>
                </div>

                {{-- PRÓXIMAS FATURAS CRÍTICAS --}}
                        ...
                        <div class="space-y-2">
                            @foreach($upcomingBills as $bill)
                                <div class="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                                    <div>
                                        <div class="text-xs font-bold text-zinc-200">{{ $bill->name }}</div>
                                        <div class="text-[9px] text-zinc-600 uppercase">
                                            Dia {{ $bill->billing_day ?? '—' }} · {{ $bill->cycle ?? 'mensal' }}
                                        </div>
                                    </div>
            ...
            {{-- ══════════════════════════════════════════
                 COLUNA CENTRAL: SURVIVAL METRICS
            ══════════════════════════════════════════ --}}
            <div class="col-span-12 lg:col-span-4 flex flex-col gap-4">

                {{-- SURVIVAL RUNWAY — WIDGET CENTRAL --}}
                <div class="anim-3 glass rounded-2xl border p-6 text-center relative overflow-hidden
                    {{ $runwayMonths >= 6 ? 'glow-green' : ($runwayMonths >= 3 ? 'glow-orange' : 'glow-red') }}"
                >
                    ...
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5">
                        <div class="w-64 h-64 rounded-full border-[40px]
                            {{ $runwayMonths >= 6 ? 'border-emerald-500' : ($runwayMonths >= 3 ? 'border-orange-500' : 'border-red-500') }}">
                        </div>
                    </div>
                    ...
                    <div class="relative z-10">
                        ...
                        <div class="tabular font-black leading-none mb-2 privacy-val
                            {{ $runwayMonths >= 6 ? 'runway-safe' : ($runwayMonths >= 3 ? 'runway-warning' : 'runway-critical') }}"
                        ...
                        <div class="text-sm font-bold text-zinc-400 mb-1">
                            meses
                            @if($runwayDays > 0)
                                <span class="text-zinc-600">e {{ $runwayDays }} dias</span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-1 mb-4">
                            @for($i = 0; $i < 12; $i++)
                                <div class="h-4 w-1.5 rounded-full transition-all duration-500
                                    ...
                                    style="transform: scaleY({{ 0.5 + ($i / 12) * 0.8 }})"
                                ></div>
                            @endfor
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-3 text-left">
                            <div class="bg-white/3 rounded-xl p-3 border border-white/5">
                                ...
                                <div class="tabular text-sm font-black text-white privacy-val">{{ number_format($totalBalance, 2, ',', '.') }}€</div>
                            </div>
                            <div class="bg-white/3 rounded-xl p-3 border border-white/5">
                                <div class="text-[8px] text-zinc-600 uppercase tracking-widest mb-1">Média Mensal</div>
                                <div class="tabular text-sm font-black text-white privacy-val">{{ number_format($avgMonthlySpend, 2, ',', '.') }}€</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="anim-4 glass rounded-2xl border border-white/5 p-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="text-center">
                            ...
                            <div class="tabular text-xl font-black privacy-val
                                {{ $burnRateActual > $burnRateDaily * 1.2 ? 'text-red-400' : ($burnRateActual > $burnRateDaily ? 'text-orange-400' : 'text-emerald-400') }}">
                                {{ number_format($burnRateActual, 2, ',', '.') }}€
                            </div>
                            <div class="text-[8px] text-zinc-700">/dia</div>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-white/5 flex items-center justify-center gap-2">
                        <span class="text-[9px] {{ $burnDiff > 0 ? 'text-red-400' : 'text-emerald-400' }}">
                            {{ $burnDiff > 0 ? '▲' : '▼' }} {{ abs($burnDiff) }}%
                        </span>
            ...
            {{-- ══════════════════════════════════════════
                 COLUNA DIREITA: BUSINESS
            ══════════════════════════════════════════ --}}
            <div class="col-span-12 lg:col-span-4 flex flex-col gap-4">

                {{-- BUSINESS DASHBOARD --}}
                <div class="anim-2 glass rounded-2xl border p-4 {{ $hasBusinessWs ? 'glow-green' : 'border-white/5' }}">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <div class="w-1.5 h-5 bg-emerald-500 rounded-full"></div>
                    ...
                    @if($hasBusinessWs)
                        ...
                        <div class="mb-4">
                            ...
                            <div class="tabular text-3xl font-black privacy-val {{ $pnl >= 0 ? 'text-emerald-400' : 'text-red-400' }}">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div class="bg-emerald-500/5 border border-emerald-500/15 rounded-xl p-3">
                                ...
                                <div class="tabular text-base font-black text-emerald-400 privacy-val">{{ number_format($monthlyRevenue, 2, ',', '.') }}€</div>
                            </div>
                            <div class="bg-red-500/5 border border-red-500/15 rounded-xl p-3">
                                ...
                                <div class="tabular text-base font-black text-red-400 privacy-val">{{ number_format($monthlyExpenses, 2, ',', '.') }}€</div>
                            </div>
                        </div>

                        <div class="mt-3 pt-3 border-t border-white/5 flex items-center justify-between">
                            <span class="text-[9px] text-zinc-500 uppercase tracking-widest">Cash Flow</span>
                            <span class="tabular text-sm font-black privacy-val {{ $cashFlow >= 0 ? 'text-emerald-400' : 'text-red-400' }}">
                                {{ $cashFlow >= 0 ? '+' : '' }}{{ number_format($cashFlow, 2, ',', '.') }}€
                            </span>
                        </div>
                    @else
                ...
                @if($hasBusinessWs)
                <div class="anim-3 glass rounded-2xl border border-white/5 p-4 {{ $overdueAR > 0 ? 'glow-orange' : '' }}">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            <div class="w-1.5 h-5 {{ $overdueAR > 0 ? 'bg-orange-500' : 'bg-zinc-600' }} rounded-full"></div>
                            <span class="text-[9px] font-black uppercase tracking-[0.3em] {{ $overdueAR > 0 ? 'text-orange-400' : 'text-zinc-500' }}">
                                Contas a Receber
                            </span>
                        </div>
                        @if($overdueAR > 0)
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <div class="tabular text-xl font-black privacy-val {{ $accountsReceivable > 0 ? 'text-white' : 'text-zinc-600' }}">
                                {{ number_format($accountsReceivable, 2, ',', '.') }}€
                            </div>
                        </div>
                        <div>
                            <div class="text-[8px] text-zinc-600 uppercase tracking-widest mb-1">Em Atraso</div>
                            <div class="tabular text-xl font-black privacy-val {{ $overdueAR > 0 ? 'text-orange-400' : 'text-zinc-600' }}">
                                {{ number_format($overdueAR, 2, ',', '.') }}€
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="anim-5 glass rounded-2xl border border-white/5 p-4">
                    ...
                    <div class="flex items-end gap-4">
                        <div>
                            ...
                            <div class="tabular text-2xl font-black text-emerald-400" x-text="'+' + ({{ $xpEarned }} + Math.floor(seconds/600)*20 - {{ $lastXpMilestone * 20 }})">
                                +{{ $xpEarned }}
                            </div>
                        </div>
                        <div class="flex-1">
                            <div class="text-[8px] text-zinc-600 uppercase tracking-widest mb-1.5">
                                Próximos <span x-text="20 - (Math.floor(seconds/600)*20 - {{ $lastXpMilestone * 20 }})">20</span> XP em
                            </div>
    ...
    <div class="anim-6 px-6 py-2.5 border-t border-white/5 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <span class="text-[8px] text-zinc-700 uppercase tracking-widest">{{ auth()->user()->name }}</span>
            <span class="text-zinc-800">·</span>
            <span class="text-[8px] text-zinc-700 uppercase tracking-widest">{{ now()->format('d/m/Y H:i') }}</span>
        </div>
        <div class="flex items-center gap-4">
